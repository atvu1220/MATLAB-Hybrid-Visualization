...
%cd(outputDirectory)
RunNumber= '183';
%Try to find the reason for the FS ion/electron difference
%plots temp,density,current for FS...and electron.
...
moverq=1.0378e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_Jperp_minusElectrons7_',RunNumber,'_',string(xcut));
...
for i=1:stepInterval:nt
    ...
    [parax,paray,paraz,perpx,perpy,perpz]=calculate_componentsVector(Bxdata_interp_y1,Bydata_interp_y1,Bzdata_interp_y1,tempx_mixed_data_interp_y1,tempy_mixed_data_interp_y1,tempz_mixed_data_interp_y1);
    tempperp_mixed_data_interp_y1 = perpx+perpy+perpz;
    temppara_mixed_data_interp_y1 = parax+paray+paraz;
...
%     Pt_mixed = n_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold)).*(temp_mixed_data_interp-ele*15).*((n_mixed_data_interp./n0 > nFS_threshold)).*10^-9.*q;
    ...
    gradn_z_mixed = (n_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)-n_mixed_data_interp(nz_min-1:nz_max-1,nx_min:nx_max));
    gradTperp_z_mixed = (tempperp_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)-tempperp_mixed_data_interp(nz_min-1:nz_max-1,nx_min:nx_max));
    gradTpara_z_mixed = (temppara_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)-temppara_mixed_data_interp(nz_min-1:nz_max-1,nx_min:nx_max));
...
%     
    ...
    hold off
    yyaxis left
    plot(smooth(n_mixed_data_interp(:,xcut)./n0),'b','linewidth',1)
    hold on
    ylim([0 0.24])
    yyaxis right
    hold on
    plot(smooth(gradn_z_mixed(2:end-1,xcut))./n0,'r','linewidth',1)
    ...
    nlegend = legend('n_{fs}','gradz n_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    xlim(xlimits)
    
    axes(ha(axes_count));axes_count=axes_count+1;
    hold off
    yyaxis left
    plot(smooth(temppara_mixed_data_interp(:,xcut)./t0),'b','linewidth',1)
    hold on
    %     ylim([0 0.24])
    yyaxis right
    hold on
    plot(smooth(gradTpara_z_mixed(2:end-1,xcut))./t0,'r','linewidth',1)
    set(gca,'ycolor','r','yminortick','on')
    ylim([ -5 5 ])
    yyaxis left
    set(gca,'ycolor','b')
    ylabel({'tpara/t0'},'FontSize',14)
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'xticklabel',[])
    nlegend = legend('Tpara_{fs}','gradz Tpara_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    xlim(xlimits)
    
    axes(ha(axes_count));axes_count=axes_count+1;
    hold off
    yyaxis left
    plot(smooth(tempperp_mixed_data_interp(:,xcut)./t0),'b','linewidth',1)
    hold on
%     ylim([0 0.24])
    yyaxis right
    hold on
    plot(smooth(gradTperp_z_mixed(2:end-1,xcut))./t0,'r','linewidth',1)
    set(gca,'ycolor','r','yminortick','on')
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    yyaxis left
    set(gca,'ycolor','b')
    ylabel({'tperp/t0'},'FontSize',14)
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'xticklabel',[])
    nlegend = legend('Tperp_{fs}','gradz Tperp_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    xlim(xlimits)
    
    
    
    axes(ha(axes_count));axes_count=axes_count+1;
    plot(smooth(upperpx_mixed(1:end,xcut)./va),'b','linewidth',1)
    hold on
    plot(smooth(electron_perpx(:,xcut)./ndata_interp(:,xcut)./va- 0*electron_perpx0(:,xcut)./ndata_interp0(:,xcut)./va),'r','linewidth',1)
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    ylabel({'Jperpx/J0'},'FontSize',14)
    xlim(zrange)
    AAAlegend = legend('vperpx_{fs}','vperpx_{e}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    xlim(xlimits)
    
    axes(ha(axes_count));axes_count=axes_count+1;
    plot(smooth(upperpy_mixed(1:end,xcut)./va),'b','linewidth',1)
    hold on
    plot(smooth(electron_perpy(:,xcut)./ndata_interp(:,xcut)./va - 0*electron_perpy0(:,xcut)./ndata_interp0(:,xcut)./va),'r','linewidth',1)
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    ylabel({'Jperpy/J0'},'FontSize',14)
    xlim(zrange)
    AAAlegend = legend('vperpy_{fs}','vperpy_{e}','Location','northwest','Orientation','horizontal'); legend('boxoff')
