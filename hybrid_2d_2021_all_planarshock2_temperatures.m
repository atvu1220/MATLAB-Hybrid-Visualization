...
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/2d_Hybrid_Foreshock_all_Run_temp',RunNumber);
...
output = outputSteps;
stepInterval = 1;
pngSlice = 0;%18 / (outputSteps*dt);
vzSpeed = -1; %if va_f=16, use i/ 3.2)
%%
gap = 20;
filterNumber = 3
...
% ylimits = [100 800];
% ylimits = [150 350];
[ndata] = read_Plasma('n_cold',nt,nx,ny,nz,outputDirectory);
display('n loaded')
...
% 
% Bxdata_interp0 = repmat(Bxdata_interp0(:,1),1,nx);
% Bxdata_interp0 = imgaussfilt(Bxdata_interp0,1);

% Bydata_interp0 = repmat(Bydata_interp0(:,1),1,nx);
% Bydata_interp0 = imgaussfilt(Bydata_interp0,1);
...
for i=startTime:stepInterval:nt
    i
    [ha,~] = tight_subplot(plot_height,plot_width,[0.02 0.01],0.1,0.1);
    ...
    axes(ha(1));
    [ndata_interp] = interpolate_Data(ndata,1,i,nx,nz,X,Z,X2,Z2);%-circshift(interpolate_Data(ndata,1,1,nx,nz,X,Z,X2,Z2),floor(i/2),1);
    ndata_interp = imgaussfilt(ndata_interp,filterNumber);
    ...
    Tdata_interp = imgaussfilt(Tdata_interp,filterNumber);
    imagesc(Tdata_interp(nz_min:nz_max,nx_min:nx_max)./t0); hold on; %shading interp
    %clear Tdata_interp
    ...
    axes(ha(3));
    [nFSdata_interp] = interpolate_Data(n_mixed_data,1,i,nx,nz,X,Z,X2,Z2)./5e15;
...
%     caxis([0.0 0.1])
   ...
   %% u cold 
    ...
    axes(ha(4));
    [pxdata_interp_cold] = (interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2))./va-MA;
    pxdata_interp_cold = imgaussfilt(pxdata_interp_cold,filterNumber);
    imagesc(pxdata_interp_cold(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp %contour(pmagdata,'color','k')
    %clear pxdata_interp
    colorbar; title('SW \Deltavx/va','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis(2*[-6 6]);ylim(ylimits)
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'XTickLabel',[])
    ylabel('Z','FontSize',14)
    ...
    axes(ha(5));
    [pydata_interp_cold] = interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2)./va;
    pydata_interp_cold = imgaussfilt(pydata_interp_cold,filterNumber);
    imagesc(pydata_interp_cold(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp%contour(pmagdata,'color','k')
    %clear pydata_interp
    colorbar; title('SW vy/va','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis(2*[-3 3]);ylim(ylimits)
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    
    %     plot6 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(6));
    [pzdata_interp_cold] = interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2)./va;
    pzdata_interp_cold = imgaussfilt(pzdata_interp_cold,filterNumber);
    imagesc(pzdata_interp_cold(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp%contour(pmagdata,'color','k')
    %clear pzdata_interp
    ...
    %% u mixed
        ...
        %     plot4 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(7));
    [pxdata_interp_mixed] = (interpolate_Data(up_mixed_data,1,i,nx,nz,X,Z,X2,Z2))./va;
    pxdata_interp_mixed = imgaussfilt(pxdata_interp_mixed,filterNumber).*((nFSdata_interp > nFS_threshold));
    imagesc(pxdata_interp_mixed(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp %contour(pmagdata,'color','k')
    %clear pxdata_interp
    colorbar; title('FS vx/va','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis(1*[-10 10]);ylim(ylimits)
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'XTickLabel',[])
    ylabel('Z','FontSize',14)
    ...
    %     plot5 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(8));
    [pydata_interp_mixed] = interpolate_Data(up_mixed_data,2,i,nx,nz,X,Z,X2,Z2)./va;
    pydata_interp_mixed = imgaussfilt(pydata_interp_mixed,filterNumber).*((nFSdata_interp > nFS_threshold));
    imagesc(pydata_interp_mixed(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp%contour(pmagdata,'color','k')
    %clear pydata_interp
    ...
    %     plot6 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(9));
    [pzdata_interp_mixed] = interpolate_Data(up_mixed_data,3,i,nx,nz,X,Z,X2,Z2)./va;
    pzdata_interp_mixed = imgaussfilt(pzdata_interp_mixed,filterNumber).*((nFSdata_interp > nFS_threshold));
    imagesc(pzdata_interp_mixed(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp%contour(pmagdata,'color','k')
    %clear pzdata_interp
    ...
    [Bxdata_interp] = (interpolate_Data(Bdata,1,i,nx,nz,X,Z,X2,Z2)-circshift(1*interpolate_Data(Bdata,1,1,nx,nz,X,Z,X2,Z2),vzSpeed*floor(i/2),1)).*moverq./(5e-9);
    Bxdata_interp = imgaussfilt(Bxdata_interp,filterNumber);
    imagesc(Bxdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp  %contour(Bmagdata,'color','k')
    colorbar; title('\DeltaBx/B0','FontSize', 14)
    %      shading interp
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis(10*[-.1 .1]);%ylim(ylimits)
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'XTickLabel',[])
    ylabel('Z','FontSize',14)
    ...
    axes(ha(11));
    [Bydata_interp] = (interpolate_Data(Bdata,2,i,nx,nz,X,Z,X2,Z2)-circshift(1*interpolate_Data(Bdata,2,1,nx,nz,X,Z,X2,Z2),vzSpeed*floor(i/2),1)).*moverq./(5e-9);
    Bydata_interp = imgaussfilt(Bydata_interp,filterNumber);
    imagesc(Bydata_interp(nz_min:nz_max,nx_min:nx_max)); hold on;%shading interp %contour(Bmagdata,'color','k')
    ...
    axes(ha(12));
    [Bzdata_interp] = interpolate_Data(Bdata,3,i,nx,nz,X,Z,X2,Z2).*moverq./(5e-9)-0*interpolate_Data(Bdata,3,1,nx,nz,X,Z,X2,Z2).*moverq./(5e-9);
    Bzdata_interp = imgaussfilt(Bzdata_interp,filterNumber);
    imagesc(Bzdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp%contour(Bmagdata,'color','k')
    colorbar; title('Bz/B0','FontSize', 14)
    %      shading interp
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis(2*[-.25 .25]);%ylim(ylimits)
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    % % %
    ...
    [parax,paray,paraz,perpx,perpy,perpz]=calculate_componentsVector(Bxdata_interp,Bydata_interp,Bzdata_interp,tempx_mixed_data_interp,tempy_mixed_data_interp,tempz_mixed_data_interp);
    temppara_mixed_data_interp = parax+paray+paraz;
    ...
    axes(ha(13));
%     [Exdata_interp] = interpolate_Data(temppara_mixed_data_interp,1,i,nx,nz,X,Z,X2,Z2)./t0;%./(va*5*1000*10^-9);
    temppara_mixed_data_interp = imgaussfilt(temppara_mixed_data_interp,filterNumber).*((nFSdata_interp > nFS_threshold));
    imagesc(temppara_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)./t0); hold on; %shading interp %contour(pmagdata,'color','k')
    %     clear Exdata_interp
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    colorbar; title('Tfs,para','FontSize', 14);%ylim(ylimits)%     shading interp
    caxis(1*[0 100])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    %set(gca,'XTickLabel',[])
    ylabel('Z','FontSize',14)
    ...
    axes(ha(14));
%     [Eydata_interp] = interpolate_Data(tempperp_mixed_data_interp,2,i,nx,nz,X,Z,X2,Z2)./t0;%./(va*5*1000*10^-9);
    tempperp_mixed_data_interp = imgaussfilt(tempperp_mixed_data_interp,filterNumber).*((nFSdata_interp > nFS_threshold));
    imagesc(tempperp_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)./t0); hold on; %shading interp%contour(pmagdata,'color','k')
    %     clear Eydata_interp
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %contour(pydata_interp(nz_min:nz_max,:)./va,'color','k')
    colorbar; title('Tfs,perp','FontSize', 14);%ylim(ylimits)
    %     shading interp
    caxis(1*[0 100])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); %set(gca,'XTickLabel',[])
    ...
    axes(ha(15));
    [temp_mixed_data_interp] =(interpolate_Data(temp_mixed_data,1,i,nx,nz,X,Z,X2,Z2))./t0;%./(va*5*1000*10^-9);
    temp_mixed_data_interp = imgaussfilt(temp_mixed_data_interp,filterNumber).*((nFSdata_interp > nFS_threshold));
    imagesc(temp_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; %shading interp%contour(pmagdata,'color','k')
    %     clear Ezdata_interp
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %contour(pzdata_interp(nz_min:nz_max,:)./va,'color','k')
    colorbar; title('Tfs','FontSize', 14);%ylim(ylimits)
    %     shading interp
    caxis(1*[0 150])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
     set(gca,'YTickLabel',[]); 
...
%     if i==pngSlice
%         fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/','2D_Run_all_',RunNumber,'_t',string(pngSlice));
%         
%         print(gcf,'-dpng','-r300','-loose',fileNamePNG);
%     end
    
    %if i==pngSlice
        fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/PlanarShock2/',RunNumber,'/2D_Run_all_temp_',RunNumber,'_t',string(i));
