...
col=3;
nParticles = 500;
nmax = 2500000%1600000;%2000000;%250000;%3500000;
...
%Desired Cell
xSC = 120*[1,1,1,1,1,1,1];%%10+[20,40,60,80];
TDthickness = 2*3;
FSbeamthickness = 100;
...
zSC = [192,201,207,210];
% zSC = [ 380,398,407,418,430]; %201 FB
zSC = [314,361,386,407,427,454] %200 HFA

% zSC = [332,360,385,410,430,450]
xSC = 120*ones(1,length(zSC));
neighboring = 1;
...
fig = figure('Position',[ 1 1  400*length(xSC) 2*800]);
% [ha,~] = tight_subplot(plot_height,plot_width,[-0.325 0],0.0,0.05);
% [ha,~] = tight_subplot(plot_height,plot_width,[-0.3525 0],0.0,0.075); %FB 201
% [ha,~] = tight_subplot(plot_height,plot_width,[-0.39 0],0.0,0.075); %FB 201b
% [ha,~] = tight_subplot(plot_height,plot_width,[-0.4625 0],0.0,0.05); %HFA 200
[ha,~] = tight_subplot(plot_height,plot_width,[-0.4775 0],0.0,0.065); %HFA 200b
...
%3 cells , 3 horizontal, 1 vertical
gridFraction = 0.5;
AreaSpace = (1+2*neighboring)*(gridFraction.^2);
Area = Area*AreaSpace;
...
for t=nt:nt
    ...
    [uex_tot] = -(interpolate_Data(uedata,1,t,nx,nz,X,Z,X2,Z2));
    [uey_tot] = -(interpolate_Data(uedata,2,t,nx,nz,X,Z,X2,Z2));
    [uez_tot] = -(interpolate_Data(uedata,3,t,nx,nz,X,Z,X2,Z2));
    
    [ufsx_tot] = (interpolate_Data(up_mixed_data,1,t,nx,nz,X,Z,X2,Z2));
    [ufsy_tot] = (interpolate_Data(up_mixed_data,2,t,nx,nz,X,Z,X2,Z2));
    [ufsz_tot] = (interpolate_Data(up_mixed_data,3,t,nx,nz,X,Z,X2,Z2));
    
    [Btcxdata_interp] = ((interpolate_Data(Btcdata,1,t,nx,nz,X,Z,X2,Z2)));
    [Btcydata_interp] = ((interpolate_Data(Btcdata,2,t,nx,nz,X,Z,X2,Z2)));
    [Btczdata_interp] = ((interpolate_Data(Btcdata,3,t,nx,nz,X,Z,X2,Z2)));
    
    [~,~,~,ueperpx,ueperpy,ueperpz] = calculate_componentsVector(Btcxdata_interp,Btcydata_interp,Btczdata_interp,...
        uex_tot,uey_tot,uez_tot);
    
    ueperpx = imgaussfilt(ueperpx,filterNumber);
    ueperpy = imgaussfilt(ueperpy,filterNumber);
    ueperpz = imgaussfilt(ueperpz,filterNumber);
    ...
    [uexBxB_x] = (interpolate_Data(uexBxBdata,1,t,nx,nz,X,Z,X2,Z2));
    [uexBxB_y] = (interpolate_Data(uexBxBdata,2,t,nx,nz,X,Z,X2,Z2));
    [uexBxB_z] = (interpolate_Data(uexBxBdata,3,t,nx,nz,X,Z,X2,Z2));
...
% % % % % % % % % % % % % %       % %5Get Cell
h = sgtitle(sprintf('time = %2.2f gyroperiods',dt*t*outputSteps),'fontsize',28);
...
%%    %First Column
    ...
    xaxisHor = calculateDistComponents([1,0,0],[xCellRange(SC,1),zCellRange(SC,1)],xDir,outputSteps,timeSteps,t,outputDirectory);
    xaxisVert = calculateDistComponents([1,0,0],[xCellRange(SC,1),zCellRange(SC,1)],yDir,outputSteps,timeSteps,t,outputDirectory);
    
    ueperpHor = (1/va).*calculateDistComponents([ueperpx(zSC(1,SC),xSC(1,SC)),ueperpy(zSC(1,SC),xSC(1,SC)),ueperpz(zSC(1,SC),xSC(1,SC))],...
        [xCellRange(SC,1),zCellRange(SC,1)],xDir,outputSteps,timeSteps,t,outputDirectory);
    ueperpVert = (1/va).*calculateDistComponents([ueperpx(zSC(1,SC),xSC(1,SC)),ueperpy(zSC(1,SC),xSC(1,SC)),ueperpz(zSC(1,SC),xSC(1,SC))],...
        [xCellRange(SC,1),zCellRange(SC,1)],yDir,outputSteps,timeSteps,t,outputDirectory);
    ufsHor = (1/va).*calculateDistComponents([ufsx_tot(zSC(1,SC),xSC(1,SC)),ufsy_tot(zSC(1,SC),xSC(1,SC)),ufsz_tot(zSC(1,SC),xSC(1,SC))],...
        [xCellRange(SC,1),zCellRange(SC,1)],xDir,outputSteps,timeSteps,t,outputDirectory);
    ufsVert = (1/va).*calculateDistComponents([ufsx_tot(zSC(1,SC),xSC(1,SC)),ufsy_tot(zSC(1,SC),xSC(1,SC)),ufsz_tot(zSC(1,SC),xSC(1,SC))],...
        [xCellRange(SC,1),zCellRange(SC,1)],yDir,outputSteps,timeSteps,t,outputDirectory);
...
%     thirdQuadrantsum
    ...
    Cphi_mixed = 1./FS_beta.*histcounts2(Vr(ParticleInCellMixed(:)==1),Vtheta(ParticleInCellMixed(:)==1),[rbins inf],[thetabins inf]);
    ...
    hold on
    quiver(-20,20,20*yaxisHor,20.*yaxisVert,'r','linewidth',3,'maxheadsize',1.5)
    quiver(-20,20,20*xaxisHor,20.*xaxisVert,'b','linewidth',3,'maxheadsize',1.5)
        
    plot(ueperpHor,ueperpVert,'kp','markersize',12,'linewidth',2)
    plot(ufsHor,ufsVert,'mp','markersize',12,'linewidth',2)
...
%     h = pcolor(X,Y,Cphi) ;
    ...
    c.FontSize=12;
    ylabel(c,'Particles per dV^2 \lambda_i^2','FontSize',18)
...
%     title(strcat('Ion Velocity Distribution at t=',string(dt*t*outputSteps),' gyroperiods'),'FontSize',14);
    ...
    xlabel({xDir;'[M_A]'},'FontSize',24);
    ylabel({yDir;'[M_A]'},'FontSize',24)
    box on
...
%     title(strcat('Ion Velocity Distribution at t=',string(dt*t*outputSteps),' gyroperiods'),'FontSize',14);
    ...
    xlabel({xDir;'[M_A]'},'FontSize',18);
    ylabel({'Bx(ExB)';'[M_A]'},'FontSize',18)
    box on
