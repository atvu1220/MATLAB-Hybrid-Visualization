...
moverq=1.0378e-8;
if pngSlice ==0
    fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/2d_Hybrid_Foreshock_PaperFig3_swfs2_',RunNumber,'_',string(xcut));%,'_100z');
...
%     plot4 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    ...
    plot(smooth(temp_cold_data_interp(:,xcut)),'b','linewidth',1.5)
    ...
    set(gca,'ycolor','b','xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    ylim([0 5.1])
    yyaxis right
    [temp_mixed_data_interp] = imgaussfilt((interpolate_Data(temp_mixed_data,1,i,nx,nz,X,Z,X2,Z2))./t0,filterNumber);
    temp_mixed_data_interp(n_mixed_data_interp < 0.001) = NaN;
     
    temp_mixed_data_interp(~isnan(temp_mixed_data_interp)) = smooth(temp_mixed_data_interp(~isnan(temp_mixed_data_interp)));
    
    
    plot((temp_mixed_data_interp(:,xcut)),'r','linewidth',1.5); hold on;
    
    [Tdata_interp] = imgaussfilt((interpolate_Data(Tdata,1,i,nx,nz,X,Z,X2,Z2))./t0,filterNumber);
    plot(smooth(Tdata_interp(:,xcut)),'k--','linewidth',1.5)
   
    
%     ylabel({'T_{fs}/T0'},'FontSize',14)
    if max(abs(ylim)) > 100
        ylim([0 max(abs(ylim))])
    else
        ylim([0 100])
    ...
    ylim([0 151])
    set(gca,'ycolor','r','xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    xlim(zrange)
    yyaxis left

    xlim(zrange)
        Tlegend = legend('sw','fs','total','Location','northwest','Orientation','horizontal','fontsize',12); legend('boxoff')
    ...
    hold off
    plot(smooth(Jxdata_interp(:,xcut)),'b','linewidth',1.5)
    hold on
...
%     ylabel({'FS Gyroradius';'[\lambda_i]'},'FontSize',14)
    ...
    if i==pngSlice
        fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/','1D_Run2',RunNumber,'_t',string(pngSlice));
