            ...
            phi = 2.0*PI/180.0
				...
				!for rotating B direction, constant Btot
					...
					!write(*,*) 'i,j,k',i,j,k,b0(i,j,k,1),b0(i,j,k,2)
                              ...
                              if (Bsetup .eq. 15) then !BLMN Coordinates,+y 60 degrees
                              !Constant B everywhere, BM
                              !b0(i,j,k,1) = 0.0
                              !b0(i,j,k,2) = 0.0
                              !b0(i,j,k,3) = 0.0
                              		if (k .le. nz/2.0) then !BL bottom
   						b0(i,j,k,1) =  (cos(60.0/180.0*pi)-cos(150.0/180.0*pi))*0.25*b0_init*eoverm +  (cos(60.0/180.0*pi)+cos(150.0/180.0*pi))*b0_init*eoverm*0.5*tanh( ( qz(nz/2.0)-qz(k))/(ddthickness*delz))
   						b0(i,j,k,2) =  (sin(60.0/180.0*pi)+sin(150.0/180.0*pi))*0.25*b0_init*eoverm +  (sin(60.0/180.0*pi)-sin(150.0/180.0*pi))*b0_init*eoverm*0.5*tanh( ( qz(nz/2.0)-qz(k))/(ddthickness*delz))
   						b0(i,j,k,3) = 0.0
					endif
					if (k .gt. nz/2.0) then !BL top
    						b0(i,j,k,1) = -(cos(60.0/180.0*pi)-cos(150.0/180.0*pi))*0.25*b0_init*eoverm +  (cos(60.0/180.0*pi)+cos(150.0/180.0*pi))*b0_init*eoverm*0.25*tanh( (qz(k)-qz(nz/2.0) )/(ddthickness*delz))
    						b0(i,j,k,2) =  (sin(60.0/180.0*pi)+sin(150.0/180.0*pi))*0.25*b0_init*eoverm + (-sin(60.0/180.0*pi)+sin(150.0/180.0*pi))*b0_init*eoverm*0.25*tanh( (qz(k)-qz(nz/2.0) )/(ddthickness*delz))
    						b0(i,j,k,3) = 0.0
					endif
					!write(*,*) 'b0_init, eoverm', b0_init, eoverm, b0_init*eoverm
					!write(*,*) 'i,j,k',i,j,k,b0(i,j,k,1),b0(i,j,k,2)
