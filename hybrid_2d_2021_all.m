...
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/2d_Hybrid_Foreshock_all_Run',RunNumber);
...
moverq=1.0378e-8;
if pngSlice == 0
    fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/2d_Hybrid_Foreshock_all_Run_',RunNumber);
...
for i=startTime:stepInterval:nt
    ...
    %clear pxdata_interp
    colorbar; title('\Deltavx/va','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    ...
    axes(ha(12));
    [Ezdata_interp] =moverq.* (interpolate_Data(Edata,3,i,nx,nz,X,Z,X2,Z2)-interpolate_Data(Edata,3,1,nx,nz,X,Z,X2,Z2))./E0;%./(va*5*1000*10^-9);
    ...
    %     clear Jxdata_interp
    colorbar; title('Jx/J0','FontSize', 14);%ylim(ylimits)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %      shading interp
    caxis(40*[-0.01 0.01])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    ...
    %     clear Jydata_interp
    colorbar; title('Jy/J0','FontSize', 14);%ylim(ylimits)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %      shading interp
    caxis(20*[-0.01 0.01]);
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    ...
    %if i==pngSlice
%         fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/2D_Run_all_',RunNumber,'_t',string(i));
%         
%         print(gcf,'-dpng','-r300','-loose',fileNamePNG);
    %end
