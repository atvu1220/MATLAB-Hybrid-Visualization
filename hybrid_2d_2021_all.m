...
for i=startTime:stepInterval:nt
    ...
    axes(ha(9));
    [Bzdata_interp] = interpolate_Data(Bdata,3,i,nx,nz,X,Z,X2,Z2).*moverq./(5e-9)-interpolate_Data(Bdata,3,1,nx,nz,X,Z,X2,Z2).*moverq./(5e-9);
    ...
    colorbar; title('\DeltaBz/B0','FontSize', 14)
    %      shading interp
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
