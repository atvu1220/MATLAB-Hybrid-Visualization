...
disp('u loaded')
[up_data] = read_Plasma('Momentum',nt,nx,ny,nz,outputDirectory);
disp('u cold loaded')
...
moverq=1.0378e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_HallTest_',RunNumber,'_',string(xcut));
...
pez_tot0 =  (ndata_interp0.*uez_tot0);
    ...
    Eswx0=Exdata_interp0./E0 + (upy_cold_data_interp0.*Bzdata_interp0-upz_cold_data_interp0.*Bydata_interp0)./va./B0;
    Eswy0=Eydata_interp0./E0 + (upz_cold_data_interp0.*Bxdata_interp0-upx_cold_data_interp0.*Bzdata_interp0)./va./B0;
    Eswz0=Ezdata_interp0./E0 + (upx_cold_data_interp0.*Bydata_interp0-upy_cold_data_interp0.*Bxdata_interp0)./va./B0;
...
%     
    ...
    [mixed_parax,mixed_paray,mixed_paraz,mixed_perpx,mixed_perpy,mixed_perpz] = ...
        calculate_componentsVector(Bxdata_interp,Bydata_interp,Bzdata_interp,...
        ppx_mixed_data_interp,ppy_mixed_data_interp,ppz_mixed_data_interp);
...
%     plot(smooth(upy_cold_data_interp(:,xcut)-upy_cold_data_interp0(:,xcut))./va,'-g','linewidth',1);
    ...
    plot(smooth(uex_tot(:,xcut))./va,'-b','linewidth',1);     hold on
    plot(smooth(uey_tot(:,xcut))./va,'-g','linewidth',1);

   yyaxis right
    %plot(smooth(upz_cold_data_interp(:,xcut)-upz_cold_data_interp0(:,xcut))./va,'-r','linewidth',1); 
    plot(smooth(uez_tot(:,xcut))./va,'-r','linewidth',1); 

    Alabel = ylabel({'u_{e,z}/va'},'FontSize',14);

        set(gca,'ycolor','k')
        yyaxis left
        set(gca,'ycolor','k')
    Alabel = ylabel({'u_{e,xy}/va'},'FontSize',14);
    hold off
    xlim(zrange)
...
%     yyaxis right
    ...
    Eswx=Exdata_interp./E0 + (0.*Bzdata_interp-0.*Bydata_interp)./va./B0 -Eswx0;
    Eswy=Eydata_interp./E0 + (0.*Bxdata_interp-10*va.*Bzdata_interp)./va./B0-Eswy0;
    Eswz=Ezdata_interp./E0 + (10*va.*Bydata_interp-0.*Bxdata_interp)./va./B0-Eswz0;%-Eprimez0;
%     Eswx=Exdata_interp./E0 + (upy_cold_data_interp0.*Bzdata_interp-upz_cold_data_interp0.*Bydata_interp)./va./B0;
%     Eswy=Eydata_interp./E0 + (upz_cold_data_interp0.*Bxdata_interp-upx_cold_data_interp0.*Bzdata_interp)./va./B0;
%     Eswz=Ezdata_interp./E0 + (upx_cold_data_interp0.*Bydata_interp-upy_cold_data_interp0.*Bxdata_interp)./va./B0;
    axes(ha(axes_count));axes_count=axes_count+1;
...
%     plot(smooth(delta_Jz_mixed(:,xcut).*delta_Ezdata_interp(:,xcut)),'-r','linewidth',1);
    plot(smooth(delta_Jx_mixed(:,xcut).*Eswx(:,xcut)),'-b','linewidth',1); hold on
    plot(smooth(delta_Jy_mixed(:,xcut).*Eswy(:,xcut)),'-g','linewidth',1);
    plot(smooth(delta_Jz_mixed(:,xcut).*Eswz(:,xcut)),'-r','linewidth',1);
    
    set(gca,'ycolor','k')
    Clabel = ylabel({'J_{fs,tot}\cdotE'},'FontSize',14);
    hold off
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'xticklabel',[])
...
%     JtotxBz = Jptotx.*delta_Bydata_interp./B0 - Jptoty.*delta_Bxdata_interp./B0;
    ...
    set(gca,'ycolor','k')
    Dlabel = ylabel({'(u_i-u_e)xB'},'FontSize',14);    
    hold off
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'xticklabel',[])
...
%     plot(smooth(delta_Ezdata_interp(:,xcut)),'-r','linewidth',1);
    ...
    %Delta E in SW Frame
%     Eswx=Exdata_interp./E0 + (0.*Bzdata_interp-0.*Bydata_interp)./va./B0;
%     Eswy=Eydata_interp./E0 + (0.*Bxdata_interp-10*va.*Bzdata_interp)./va./B0;
%     Eswz=Ezdata_interp./E0 + (10*va.*Bydata_interp-0.*Bxdata_interp)./va./B0;


    axes(ha(axes_count));axes_count=axes_count+1;
    plot(smooth(Eswx(:,xcut)),'-b','linewidth',1); hold on
    plot(smooth(Eswy(:,xcut)),'-g','linewidth',1);
    plot(smooth(Eswz(:,xcut)),'-r','linewidth',1);
 
    set(gca,'ycolor','k')
    Flabel = ylabel("E_{sw}/E0",'FontSize',14);
    hold off
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'xticklabel',[])
    Flegend = legend('x','y','z','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
...
%     JtotxBz = (upx_data_interp-uex_tot).*Bydata_interp./B0./va - (upy_data_interp-uey_tot).*Bxdata_interp./B0./va;
    ...
    set(gca,'ycolor','k')
    Elabel = ylabel({'uixB'},'FontSize',14)  ;
    hold off
    xlim(zrange)
    ...
    set(gca,'ycolor','k')
    Elabel = ylabel({'uexB'},'FontSize',14)  ;
    hold off
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5)
