...
moverq=1.0378e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_Jperp_separate',RunNumber,'_',string(xcut));
...
pex_cold0 = ndata_interp0.*uex_cold0;
pey_cold0 = ndata_interp0.*uey_cold0;
pez_cold0 = ndata_interp0.*uez_cold0;
...
%     EBmag = (Exdata_interp.^2 + Eydata_interp.^2 + Ezdata_interp.^2)./(1/2).*(Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2).^(1/2);
    ...
    delta_upparax_electron = (upparax_electron-del*upparax_electron0)* 10^3*10^-9.*q./nv0;
    delta_upparay_electron = (upparay_electron-del*upparay_electron0)*  10^3*10^-9.*q./nv0;
    delta_upparaz_electron = (upparaz_electron-del*upparaz_electron0)*  10^3*10^-9.*q./nv0;
    delta_upperpx_electron = (upperpx_electron-del*upperpx_electron0)*  10^3*10^-9.*q./nv0;
    delta_upperpy_electron = (upperpy_electron-del*upperpy_electron0)*  10^3*10^-9.*q./nv0;
    delta_upperpz_electron = (upperpz_electron-del*upperpz_electron0)*  10^3*10^-9.*q./nv0;
...
%     uez_mixed = upz_mixed_data_interp - curlBz_interp*moverq./ (1000*4*pi*10^-7 .* q .* (n_mixed_data_interp)*10^-9)./1000; %in km/s
    ...
    delta_mixed_parax = (mixed_parax-del*mixed_parax0).*10^3 * 10^-9.*q./nv0;
    delta_mixed_paray = (mixed_paray-del*mixed_paray0).*10^3 * 10^-9.*q./nv0;
    delta_mixed_paraz = (mixed_paraz-del*mixed_paraz0).*10^3 * 10^-9.*q./nv0;
    delta_mixed_perpx = (mixed_perpx-del*mixed_perpx0).*10^3 * 10^-9.*q./nv0;
    delta_mixed_perpy = (mixed_perpy-del*mixed_perpy0).*10^3 * 10^-9.*q./nv0;
    delta_mixed_perpz = (mixed_perpz-del*mixed_perpz0).*10^3 * 10^-9.*q./nv0;
...
Jperpx_tot = delta_cold_perpx(2:end-1,2:end-1) + delta_mixed_perpx(2:end-1,2:end-1) - delta_upperpx_electron(2:end-1,2:end-1);
Jperpy_tot = delta_cold_perpy(2:end-1,2:end-1) + delta_mixed_perpy(2:end-1,2:end-1) - delta_upperpy_electron(2:end-1,2:end-1);
...
%     plot(smooth(jDx_cold(:,xcut)),'linewidth',1)
    ...
    plot(smooth(delta_upperpy_electron(:,xcut)),'linewidth',1)
    hold off
    ylabel({'Jperpx/J0'},'FontSize',14)
    xlim(zrange)
    Alegend = legend('\Deltanqv_{sw}','\Deltanqv_{fs}','\Deltanqv_e','Location','northwest','Orientation','horizontal'); legend('boxoff')
...
%     plot(smooth(jDy_cold(:,xcut)),'linewidth',1)
    ...
    xlim(zrange)
    Blegend = legend('\Deltanqv_{sw}','\Deltanqv_{fs}','\Deltanqv_e','Location','northwest','Orientation','horizontal'); legend('boxoff')
