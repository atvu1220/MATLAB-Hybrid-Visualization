...
fig = figure('Position',[ 1 1 850 750]);
outputFolder = '146';
...
moverq=1.044e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_PaperFig1_',RunNumber);
...
%     plot2 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    ...
    Tdata_interp = imgaussfilt(Tdata_interp,filterNumber);
    pcolor(Tdata_interp(nz_min:nz_max,nx_min:nx_max)./t0); hold on; shading interp
    %clear Tdata_interp
...
%     axes(ha(10));
%     [Exdata_interp] = interpolate_Data(Edata,1,i,nx,nz,X,Z,X2,Z2);%./(va*5*1000*10^-9);
...
%     %     clear Exdata_interp
%     set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
%     colorbar; title('Ex','FontSize', 14);%ylim(ylimits)%     shading interp
%     caxis([-25 25])
%     set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
%     set(gca,'XTickLabel',[])
%     ylabel('Z','FontSize',14)
...
%     axes(ha(11));
%     [Eydata_interp] = interpolate_Data(Edata,2,i,nx,nz,X,Z,X2,Z2);%./(va*5*1000*10^-9);
...
%     colorbar; title('Ey','FontSize', 14);%ylim(ylimits)
%     %     shading interp
%     caxis([-25 25])
%     set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
%     set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
...
%     colorbar; title('Ez-Ez_0','FontSize', 14);%ylim(ylimits)
%     %     shading interp
%     caxis([-25 25])
%     set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
%     set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
...
%     Jxdata_interp = conv2(Jxdata_interp,Jxdata_interp,'same');
    ...
    pcolor(Jxdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear Jxdata_interp
    colorbar; title('\DeltaJx','FontSize', 14);%ylim(ylimits)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %      shading interp
     caxis([-max(abs(caxis)) max(abs(caxis))])
%     caxis([-6 6])
...
%     clear Jydata_interp
    colorbar; title('\DeltaJy','FontSize', 14);%ylim(ylimits)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %      shading interp
     caxis([-max(abs(caxis)) max(abs(caxis))])
%     caxis([-6 6]);
...
%     plot15 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(9));
    [Jzdata_interp] = interpolate_Data(Jdata,3,i,nx,nz,X,Z,X2,Z2)-interpolate_Data(Jdata,3,1,nx,nz,X,Z,X2,Z2);%.*ndata_interp.*alpha;
...
%     clear Jzdata_interp
    colorbar; title('\DeltaJz','FontSize', 14);%ylim(ylimits)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %      shading interp
%     caxis([-3 3])
 caxis([-max(abs(caxis)) max(abs(caxis))])
    ...
    fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/',RunNumber,'/','2d_Hybrid_Foreshock_paperFig1_',RunNumber,'_',string(i));
