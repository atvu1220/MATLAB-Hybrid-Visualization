...
fig = figure('Position',[ 1 1 250+8*250 2.5*250]);
outputFolder = '180';
...
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_PaperFig2f_',RunNumber);
...
%Parallel Speed 
ppx_mixed_bulk = -2*FS_Ma*Bxdata_interp0 ./ (Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2).^(1/2);
ppy_mixed_bulk = -2*FS_Ma*Bydata_interp0 ./ (Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2).^(1/2);
ppz_mixed_bulk = -2*FS_Ma*Bzdata_interp0 ./ (Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2).^(1/2);

for k=nz/2+1:nz
    ppx_mixed_bulk(k,:) = 0.0;%-2*FS_Ma*Bxdata_interp0(nz/2,:) ./ (Bxdata_interp0(nz/2,:).^2 + Bydata_interp0(nz/2,:).^2 + Bzdata_interp0(nz/2,:).^2).^(1/2);
    ppy_mixed_bulk(k,:) = 0.0;%-2*FS_Ma*Bydata_interp0(nz/2,:) ./ (Bxdata_interp0(nz/2,:).^2 + Bydata_interp0(nz/2,:).^2 + Bzdata_interp0(nz/2,:).^2).^(1/2);
    ppz_mixed_bulk(k,:) = 0.0;%-2*FS_Ma*Bzdata_interp0(nz/2,:) ./ (Bxdata_interp0(nz/2,:).^2 + Bydata_interp0(nz/2,:).^2 + Bzdata_interp0(nz/2,:).^2).^(1/2);
end

ppx_mixed_bulk = ppx_mixed_bulk + vExB_x;
ppy_mixed_bulk = ppy_mixed_bulk + vExB_y;
...
[ppx_cold_data_interp0] = 0.996.*n_cold_data_interp0.*upx_cold_data_interp0;
[ppy_cold_data_interp0] = 0.996.*n_cold_data_interp0.*upy_cold_data_interp0;
[ppz_cold_data_interp0] = 0.996.*n_cold_data_interp0.*upz_cold_data_interp0;
[pparacold0,pperpcold0] = calculate_components(Bxdata_interp0,Bydata_interp0,Bzdata_interp0,ppx_cold_data_interp0,ppy_cold_data_interp0,ppz_cold_data_interp0);
...
for i=1:stepInterval:nt
    ...
    [upx_cold_data_interp] = interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2)./va - 0.996.*upx_cold_data_interp0./va;
   ...
   %% Cold Para/Perp
    [ppx_cold_data_interp] = n_cold_data_interp.*imgaussfilt((interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2)),filterNumber);
    [ppy_cold_data_interp] = n_cold_data_interp.*imgaussfilt((interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2)),filterNumber);
    [ppz_cold_data_interp] = n_cold_data_interp.*imgaussfilt((interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2)),filterNumber);
    ...
    colorbar; title('SW \Deltanqvperpx/J0','FontSize', 14);
    ...
    colorbar; title('SW \Deltanqvperpy/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    ...
    Pt_cold = n_cold_data_interp.*temp_cold_data_interp.*10^-9.*q;
    ...
    Pt_cold_y1 = n_cold_data_interp_y1.*temp_cold_data_interp_y1.*10^-9.*q;
    gradP_y_cold = (Pt_cold(nz_min:nz_max,nx_min:nx_max)-Pt_cold_y1(nz_min:nz_max,nx_min:nx_max))./((lambda_i)*1000);
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(gradP_x_cold); shading interp
    colorbar; title('SW gradPx/n0T0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-0.25*max(abs(caxis)) 0.25*max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(gradP_z_cold); shading interp
    colorbar; title('SW gradPz/n0T0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-0.5*max(abs(caxis)) 0.5*max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(jDx_cold); shading interp
    colorbar; title('SW jDx/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(jDy_cold); shading interp
    colorbar; title('SW jDy/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    %% XYZ Mixed
    %Calculate ExB from SW
    usw_x = imgaussfilt(interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2),filterNumber);
    usw_y = imgaussfilt(interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2),filterNumber);
    usw_z = imgaussfilt(interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    Ex = Bydata_interp.*usw_z - Bzdata_interp.*usw_y;
    Ey = Bzdata_interp.*usw_x - Bxdata_interp.*usw_z;
    Ez = Bxdata_interp.*usw_y - Bydata_interp.*usw_x;
    vExBx = (Bzdata_interp.*Ey - Bydata_interp.*Ez)./(Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2);
    vExBy = (Bxdata_interp.*Ez - Bzdata_interp.*Ex)./(Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2);
    vExBz = (Bydata_interp.*Ex - Bxdata_interp.*Ey)./(Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2);
    
    axes(ha(axes_count));axes_count=axes_count+1;
    [upx_mixed_data_interp] = (interpolate_Data(up_mixed_data,1,i,nx,nz,X,Z,X2,Z2))- vExBx;
    upx_mixed_data_interp = upx_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold))./va...
        -ppx_mixed_bulk.*((n_mixed_data_interp./n0 > nFS_threshold));%2*FS_Ma.*((n_mixed_data_interp./n0 > nFS_threshold));
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    [upy_mixed_data_interp] = interpolate_Data(up_mixed_data,2,i,nx,nz,X,Z,X2,Z2) - vExBy;
    upy_mixed_data_interp = upy_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold))./va...
        -ppy_mixed_bulk.*((n_mixed_data_interp./n0 > nFS_threshold));
    ...
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[]); 
    %% Mixed Para/Perp
    [ppx_mixed_data_interp] = n_mixed_data_interp.*(imgaussfilt(interpolate_Data(up_mixed_data,1,i,nx,nz,X,Z,X2,Z2),filterNumber)-vExBx);
    ppx_mixed_data_interp= ppx_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold));
    [ppy_mixed_data_interp] = n_mixed_data_interp.*(imgaussfilt(interpolate_Data(up_mixed_data,2,i,nx,nz,X,Z,X2,Z2),filterNumber)-vExBy);
    ppy_mixed_data_interp= ppy_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold));
    [ppz_mixed_data_interp] = n_mixed_data_interp.*(imgaussfilt(interpolate_Data(up_mixed_data,3,i,nx,nz,X,Z,X2,Z2),filterNumber)-vExBz);
    ppz_mixed_data_interp= ppz_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold));
    
    ppx_mixed_data_interp0 = 0.12*n0.*ppx_mixed_bulk.*va.*((n_mixed_data_interp./n0 > nFS_threshold));
    ppy_mixed_data_interp0 = 0.12*n0.*ppy_mixed_bulk.*va.*((n_mixed_data_interp./n0 > nFS_threshold));
    ppz_mixed_data_interp0 = 0.12*n0.*ppz_mixed_bulk.*va.*((n_mixed_data_interp./n0 > nFS_threshold));
    ...
    colorbar; title('FS \Deltanqvperpx/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    ...
    colorbar; title('FS \Deltanqvperpy/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]); set(gca,'XTickLabel',[])
    ...
    Pt_mixed = n_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold)).*temp_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold)).*10^-9.*q;
    ...
    Pt_mixed_y1 = n_mixed_data_interp_y1.*((n_mixed_data_interp_y1./n0 > nFS_threshold)).*temp_mixed_data_interp_y1.*((n_mixed_data_interp_y1./n0 > nFS_threshold)).*10^-9.*q;
    gradP_y_mixed = (Pt_mixed(nz_min:nz_max,nx_min:nx_max)-Pt_mixed_y1(nz_min:nz_max,nx_min:nx_max))./((lambda_i)*1000);
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(gradP_x_mixed); shading interp
    colorbar; title('FS gradPx/n0T0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-0.25*max(abs(caxis)) 0.25*max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[])
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(gradP_z_mixed); shading interp
    colorbar; title('FS gradPz/n0T0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-0.5*max(abs(caxis)) 0.5*max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[])
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(jDx_mixed); shading interp
    colorbar; title('FS jDx/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[])
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(jDy_mixed); shading interp
    colorbar; title('FS jDy/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[])
   %% XYZ Currents
    ...
    delta_Jxdata = (Jxdata_interp - Jxdata_interp0).*moverq./J0;
    delta_Jydata = (Jydata_interp - Jydata_interp0).*moverq./J0;
    
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(delta_Jxdata(nz_min:nz_max,nx_min:nx_max)); shading interp
    colorbar; title('\DeltaJx/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(delta_Jydata(nz_min:nz_max,nx_min:nx_max)); shading interp
    colorbar; title('\DeltaJy/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[])
    ...
    colorbar; title('\DeltaJperpx/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[])
    ...
    colorbar; title('\DeltaJperpy/J0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[])
    xlabel({'X';'[\lambda_i]'},'FontSize',14)
    %% Perp Para Thermal Pressure Gradient
%     [Jdparax_cold,Jdparay_cold,Jdparaz_cold,Jdperpx_cold,Jdperpy_cold,Jdperpz_cold]       = calculate_componentsVector(Bxdata_interp(nz_min:nz_max,nx_min:nx_max),Bydata_interp(nz_min:nz_max,nx_min:nx_max),Bzdata_interp(nz_min:nz_max,nx_min:nx_max),...
%         jDx_cold,jDy_cold,zeros(size(Bzdata_interp(nz_min:nz_max,nx_min:nx_max))));
%     [Jdparax_mixed,Jdparay_mixed,Jdparaz_mixed,Jdperpx_mixed,Jdperpy_mixed,Jperpz_mixed] = calculate_componentsVector(Bxdata_interp(nz_min:nz_max,nx_min:nx_max),Bydata_interp(nz_min:nz_max,nx_min:nx_max),Bzdata_interp(nz_min:nz_max,nx_min:nx_max),...
%     jDx_mixed,jDy_mixed,zeros(size(Bzdata_interp(nz_min:nz_max,nx_min:nx_max))));
% 
%     axes(ha(axes_count));axes_count=axes_count+1;
%     pcolor(Jdperpx_cold);  shading interp
%     colorbar; title('SW Jdperpx','FontSize', 14)
%     caxis([-max(abs(caxis)) max(abs(caxis))])
%     set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
%     set(gca,'YTickLabel',[]); xlabel({'X';'[\lambda_i]'},'FontSize',14)
%     
%     axes(ha(axes_count));axes_count=axes_count+1;
%     pcolor(Jdperpy_cold);  shading interp
%     colorbar; title('SW Jdperpy','FontSize', 14)
%     caxis([-max(abs(caxis)) max(abs(caxis))])
%     set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
%     set(gca,'YTickLabel',[]); xlabel({'X';'[\lambda_i]'},'FontSize',14)
%     
%     axes(ha(axes_count));axes_count=axes_count+1;
%     pcolor(Jdperpx_mixed);  shading interp
%     colorbar; title('FS Jdperpx','FontSize', 14)
%     caxis([-max(abs(caxis)) max(abs(caxis))])
%     set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
%     set(gca,'YTickLabel',[]); xlabel({'X';'[\lambda_i]'},'FontSize',14)
%     
%     axes(ha(axes_count));axes_count=axes_count+1;
%     pcolor(Jdperpy_mixed);  shading interp
%     colorbar; title('FS Jdperpy','FontSize', 14)
%     caxis([-max(abs(caxis)) max(abs(caxis))])
%     set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
%     set(gca,'YTickLabel',[]); xlabel({'X';'[\lambda_i]'},'FontSize',14)
    %% Densities & Temperatures
    axes(ha(axes_count));axes_count=axes_count+1; %#ok<*LAXES>
    pcolor(n_cold_data_interp(nz_min:nz_max,nx_min:nx_max)./n0); shading interp
    colorbar; title('SW n/n0','FontSize', 14)
    caxis([0 2])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits);
    set(gca,'YTickLabel',[]);xlabel({'X';'[\lambda_i]'},'FontSize',14)
    
    axes(ha(axes_count));axes_count=axes_count+1; %#ok<*LAXES>
    pcolor(n_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)./n0); shading interp
    colorbar; title('FS n/n0','FontSize', 14)
    caxis([0 0.24])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
    set(gca,'YTickLabel',[]);xlabel({'X';'[\lambda_i]'},'FontSize',14)
    
    axes(ha(axes_count));axes_count=axes_count+1;
    pcolor(temp_cold_data_interp(nz_min:nz_max,nx_min:nx_max)./t0); shading interp
    ...
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits); xlabel({'X';'[\lambda_i]'},'FontSize',14)
    set(gca,'YTickLabel',[])
    
    axes(ha(axes_count));axes_count=axes_count+1;
    temp_mixed_data_interp=temp_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold));
    pcolor(temp_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)./t0);  shading interp
    ...
    %% Draw
    drawnow
    fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/',RunNumber,'/','2d_Hybrid_Foreshock_paperFig2c_',RunNumber,'_',string(i));
