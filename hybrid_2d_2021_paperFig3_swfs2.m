...
pngSlice =0;%10/(outputSteps *dt);% 6 / (outputSteps * dt);
%%
...
zrange = [(zmid-ddthickness-foreshockthickness) (zmid+ddthickness+foreshockthickness)];
zrange = [nz/2 - 150 , nz/2 + 150]-2;%[100 300];%[200 400]
zrange = [ (nz)/2-ddthickness, (nz)/2  + ddthickness];
zrange = [151 249]
...
moverq=1.0378e-8;
if pngSlice ==0
    fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/2d_Hybrid_Foreshock_PaperFig3_swfsJ_',RunNumber,'_',string(xcut));%,'_100z');
...
for i=startTime:stepInterval:nt
    ...
    axes(ha(1));
    [Bxdata_interp] = imgaussfilt((interpolate_Data(Bdata,1,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(Bdata,1,1,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9),filterNumber);
    [Bydata_interp] = imgaussfilt((interpolate_Data(Bdata,2,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(Bdata,2,1,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9),filterNumber);
    [Bzdata_interp] = imgaussfilt((interpolate_Data(Bdata,3,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(Bdata,3,1,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9),filterNumber);
    Bmagdata = (Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2).^(1/2);
    hold off;
    plot(smooth(Bxdata_interp(:,xcut)),'b','linewidth',1.5)
    hold on
    plot(smooth(Bydata_interp(:,xcut)),'g','linewidth',1.5)
    plot(smooth(Bzdata_interp(:,xcut)),'r','linewidth',1.5)
    plot(Bmagdata(:,xcut),'--k','linewidth',1.5)
    hold off
    ...
    %     ylim([-0.5 0.5])
    %     ylim([-1 1])
    Blegend = legend('Bx','By','Bz','Btotal','Location','northwest','Orientation','horizontal','fontsize',12); legend('boxoff')
    ...
    [n_cold_data_interp] = imgaussfilt((interpolate_Data(n_cold_data,1,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(n_cold_data,1,1,nx,nz,X,Z,X2,Z2))./n0,filterNumber);
    ...
    hold off
    yyaxis left
    plot(smooth((n_cold_data_interp(:,xcut))),'b','linewidth',1.5)
    hold on
    plot(smooth(1+(ndata_interp(:,xcut))),'--k','linewidth',1.5)
    ...
    %         plot(smooth((n_mixed_data_interp(:,xcut)))./smooth((n_cold_data_interp2(:,xcut))) ,'r','linewidth',1)
    ...
    axes(ha(3));
    hold on
    [pxdata_interp] = imgaussfilt((interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_cold_data,1,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    [pydata_interp] = imgaussfilt((interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_cold_data,2,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    [pzdata_interp] = imgaussfilt((interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_cold_data,3,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    hold off
    plot(smooth(pxdata_interp(:,xcut)),'b','linewidth',1.5)
    hold on
    plot(smooth(pydata_interp(:,xcut)),'g','linewidth',1.5)
    plot(smooth(pzdata_interp(:,xcut)),'r','linewidth',1.5)
    hold off
    ylabel({'SW v/va'},'FontSize',14)
    ...
    ylim([-3 11])
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    vlegend = legend('vx','vy','vz','Location','west','Orientation','horizontal','fontsize',12); legend('boxoff')
    
    
    
    
    %     [upx_cold_data_interp] = imgaussfilt((interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_cold_data,1,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    %     [upy_cold_data_interp] = imgaussfilt((interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_cold_data,2,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    %     [upz_cold_data_interp] = imgaussfilt((interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_cold_data,3,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    %     hold off
    %     plot(smooth(upx_cold_data_interp(:,xcut)),'b','linewidth',1)
    %     hold on
    %     plot(smooth(upy_cold_data_interp(:,xcut)),'g','linewidth',1)
    %     plot(smooth(upz_cold_data_interp(:,xcut)),'r','linewidth',1)
    %     hold off
    %     ylabel({'u_{sw}/va'},'FontSize',14)
    %
    %     xlim(zrange)
    %     %     if max(abs(ylim)) > 15
    %     %         ylim([-3 max(abs(ylim))])
    ...
    %     ylim([-2 11])
    %     set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    %     vlegend = legend('ux','uy','uz','Location','west','Orientation','horizontal','fontsize',12); legend('boxoff')
    ...
    % axes(ha(5));
    % hold on
    [upx_mixed_data_interp] = imgaussfilt((interpolate_Data(up_mixed_data,1,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_mixed_data,1,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    [upy_mixed_data_interp] = imgaussfilt((interpolate_Data(up_mixed_data,2,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_mixed_data,2,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    [upz_mixed_data_interp] = imgaussfilt((interpolate_Data(up_mixed_data,3,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(up_mixed_data,3,1,nx,nz,X,Z,X2,Z2))./va,filterNumber);
    % hold off
    % plot(smooth(upx_mixed_data_interp(:,xcut).*((n_mixed_data_interp(:,xcut) > nFS_threshold))),'b','linewidth',1)
    % hold on
    % plot(smooth(upy_mixed_data_interp(:,xcut).*((n_mixed_data_interp(:,xcut) > nFS_threshold))),'g','linewidth',1)
    % plot(smooth(upz_mixed_data_interp(:,xcut).*((n_mixed_data_interp(:,xcut) > nFS_threshold))),'r','linewidth',1)
    % hold off
    % ylabel({'u_{fs}/va'},'FontSize',14)
    %
    % xlim(zrange)
    % %     if max(abs(ylim)) > 15
    % %         ylim([-3 max(abs(ylim))])
    ...
    % ylim([-22 11])
    % set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    % vvlegend = legend('ux','uy','uz','Location','west','Orientation','horizontal','fontsize',12); legend('boxoff')
    % %     xlabel({'Z','[\lambda_i]'},'FontSize',14)
    %
    
    %     plot4 = subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(4));
    upx_mixed_data_interp(n_mixed_data_interp < 0.001) = NaN;
    upy_mixed_data_interp(n_mixed_data_interp < 0.001) = NaN;
    upz_mixed_data_interp(n_mixed_data_interp < 0.001) = NaN;

    plot(upx_mixed_data_interp(:,xcut),'b','linewidth',1.5)
    hold on
    plot(upy_mixed_data_interp(:,xcut),'g','linewidth',1.5)
    plot(upz_mixed_data_interp(:,xcut),'r','linewidth',1.5)
    hold off
    ylabel({'FS v/va'},'FontSize',14)
    ...
    [ppx_mixed_data_interp] = n_mixed_data_interp.*(upx_mixed_data_interp.*va).*n0;%.*((n_mixed_data_interp./n0 > nFS_threshold));
    [ppy_mixed_data_interp] = n_mixed_data_interp.*(upy_mixed_data_interp.*va).*n0;%.*((n_mixed_data_interp./n0 > nFS_threshold));
    [ppz_mixed_data_interp] = n_mixed_data_interp.*(upz_mixed_data_interp.*va).*n0;%.*((n_mixed_data_interp./n0 > nFS_threshold));
    ...
    pex_mixed = n_mixed_data_interp .* uex_tot.*n0;
    pey_mixed = n_mixed_data_interp .* uey_tot.*n0;
    pez_mixed = n_mixed_data_interp .* uez_tot.*n0;
...
%     [Jzdata_interp] = imgaussfilt((interpolate_Data(Jdata,3,i,nx,nz,X,Z,X2,Z2))./J0,filterNumber+1);%.*ndata_interp.*alpha.*n0;

    plot(Jperpx_mixed(:,xcut),'b','linewidth',1.5)
    hold on
    plot(Jperpy_mixed(:,xcut),'g','linewidth',1.5)
    plot(del_Jperp(:,xcut),'k','linewidth',1.5)
hold off
%     plot(smooth(Jydata_interp(:,xcut)),'g','linewidth',1.5)
%     plot(smooth(Jzdata_interp(:,xcut)),'r','linewidth',1.5)
%     hold off
    ylabel({'FS Jperp/J0'},'FontSize',14)
    xlim(zrange)
    if max(abs(ylim)) > .001
...
%     ylim([-0.61 +0.61])
    ...
    xlabel({'Z','[\lambda_i]'},'FontSize',14)
    Jlegend = legend({'x';'y';'tot'},'Location','southwest','Orientation','horizontal','fontsize',12); legend('boxoff')
    ...
    %sign
    vpara = upx_mixed_data_interp(:,xcut).*Bxdata_interp(:,xcut)./Bmagdata(:,xcut) +...
        upy_mixed_data_interp(:,xcut).*Bydata_interp(:,xcut)./Bmagdata(:,xcut) +...
        upz_mixed_data_interp(:,xcut).*Bzdata_interp(:,xcut)./Bmagdata(:,xcut);
    vperp =( (upx_mixed_data_interp(:,xcut) - vpara(:).*Bxdata_interp(:,xcut)./Bmagdata(:,xcut)).^2 + ...
        (upy_mixed_data_interp(:,xcut) - vpara(:).*Bydata_interp(:,xcut)./Bmagdata(:,xcut)).^2 + ...
        (upz_mixed_data_interp(:,xcut) - vpara(:).*Bzdata_interp(:,xcut)./Bmagdata(:,xcut)).^2 ).^(1/2);
    
    gyroradius = 1.67e-27 .* vperp*va ./ (1.601e-19 .*Bmagdata(:,xcut)*5e-9 )    ;
    
    gyroradius(n_mixed_data_interp(:,xcut) < 0.001) = NaN;
    gyroradius(~isnan(gyroradius)) = smooth(gyroradius(~isnan(gyroradius)));
    
    vperp(n_mixed_data_interp(:,xcut) < 0.001) = NaN;
    vperp(~isnan(vperp)) = smooth(vperp(~isnan(vperp)));
    
    vpara(n_mixed_data_interp(:,xcut) < 0.001) = NaN;
    vpara(~isnan(vpara)) = smooth(vpara(~isnan(vpara)));
    
    
    axes(ha(5));
    yyaxis left
    plot([1:length(vperp)],(vperp(:)),'b','linewidth',1.5)
    ylabel({'FS Vperp/va'},'FontSize',14)
    %     if max(abs(ylim)) > 1.5
    %         ylim([0 max(abs(ylim))])
    %     else
    %         ylim([0 1.5])
    %     end
    set(gca,'ycolor','b','xminortick','on','yminortick','on','linewidth',1.5)
    ylim([0 25])
    
    yyaxis right
    plot([1:length(vpara)],(vpara),'r','linewidth',1.5); hold on;
    set(gca,'ycolor','r','xminortick','on','yminortick','on','linewidth',1.5)
    
    xlim(zrange)
    ylabel({'FS Vpara/va'},'FontSize',14)
    %     ylabel({'FS Gyroradius';'[\lambda_i]'},'FontSize',14)
    ylim([-25 0])
...
%     xlabel({'Z','[\lambda_i]'},'FontSize',14)
    ...
    fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/',RunNumber,'/','2d_Hybrid_Foreshock_paperFig3_FSJ_',RunNumber,'_',string(i));
    ...
    if i==pngSlice
        fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/','1D_RunFSJ',RunNumber,'_t',string(pngSlice));
