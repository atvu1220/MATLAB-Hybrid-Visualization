...
ele=0;
del = 1;
JdotE_mixed_minimum = 0;
%% Load Dist
numproc = 24;
nmax = 6000000;%400000;%250000;%3500000;%number of particles Ni_max
...
mixed=single(zeros(1,numproc,nmax,1));
x=single(zeros(1,numproc,nmax,3));
...
allv= reshape(v,[1,numproc*nmax,3]); clear v
allx = reshape(x,[1,numproc*nmax,3]); clear x
allmixed = reshape(mixed,[1,numproc*nmax,1]); clear mixed

allv_bot_foreshock = allv(1,allv(1,:,1) < 0 & allmixed(1,:) == 0,:);
allv_top_foreshock = allv(1,allv(1,:,1) < 0 & allmixed(1,:) == 1,:);


allx_bot_foreshock = allx(1,allv(1,:,1) < 0 & allmixed(1,:) == 0,:);
allx_top_foreshock = allx(1,allv(1,:,1) < 0 & allmixed(1,:) == 1,:);


allv_foreshock = allv(1,allv(1,:,1) < 0 ,:);
allv_solarwind =  allv(1,allv(1,:,1) >= 0 ,:);

allx_foreshock = allx(1,allv(1,:,1) < 0 ,:);
allx_solarwind =  allx(1,allv(1,:,1) >= 0 ,:);
%% Interpolate for moments
dx=lambda_i;
dy=lambda_i;
dz=lambda_i;
ppc=96;
procnum=24;
nf_init = 5e15;
Ni_tot = length(reshape(allx,[numproc*nmax,3]));
ijkp=zeros(Ni_tot,3);
beta_particle = 1.0;
beta_p = beta_particle * ones(Ni_tot,1);
Ni_tot_sys = floor((nx-2)*(ny-2)*(nz-2)*(ppc/procnum))*procnum;

[wght,ijkp] = get_interp_weights(ijkp,reshape(allx,[numproc*nmax,3]),Ni_tot,qx,qy,qz,nx,ny,nz,dx,dy,dz);
[beta] = get_beta(Ni_tot_sys,nf_init,qx,qy,qz,nx,ny,nz);
[dx_grid,dy_grid,dz_grid,qx,qy,qz] = grd7(nx,ny,nz,dx,dy,dz);

[np] = update_np(beta,beta_p,ijkp,reshape(allx,[numproc*nmax,3]),Ni_tot,wght,...
    nx,ny,nz,...
    dx_grid,dy_grid,dz_grid,...
    qx,qy,qz,...
    dx,dy,dz);
[up] = update_up(beta,beta_p,ijkp,np,reshape(allv,[numproc*nmax,3]),Ni_tot,wght,nx,ny,nz,qx,qy,qz);
% [ndata] = read_Plasma('n',nt,nx,ny,nz,outputDirectory);
% disp('n loaded')
% 
% [udata] = read_Plasma('Momentum',nt,nx,ny,nz,outputDirectory);
% display('v loaded')
% nreal = reshape(ndata(target_time,:,2,:),[nx nz]);
ncalc = reshape(np(:,2,:),[nx nz]);
% uxreal = reshape(udata(target_time,:,2,:,1),[nx nz]);
% uyreal = reshape(udata(target_time,:,2,:,2),[nx nz]);
% uzreal = reshape(udata(target_time,:,2,:,3),[nx nz]);
uxcalc = reshape(up(:,2,:,1),[nx nz]);
uycalc = reshape(up(:,2,:,2),[nx nz]);
uzcalc = reshape(up(:,2,:,3),[nx nz]);
%data_interp=reshape(Data(t,:,2,:,component),[nx,nz]);%-reshape(Data(1,:,2,:,component),[nx,nz]);
...
procnum=24;
nf_init = 5e15;
Ni_tot = length(allx_solarwind);
ijkp=zeros(Ni_tot,3);
beta_particle = 1.0;
beta_p = beta_particle * ones(Ni_tot,1);
Ni_tot_sys = floor((nx-2)*(ny-2)*(nz-2)*(ppc/procnum))*procnum;

[wght,ijkp] = get_interp_weights(ijkp,reshape(allx_solarwind,[length(allx_solarwind),3]),Ni_tot,qx,qy,qz,nx,ny,nz,dx,dy,dz);
[beta] = get_beta(Ni_tot_sys,nf_init,qx,qy,qz,nx,ny,nz);
[dx_grid,dy_grid,dz_grid,qx,qy,qz] = grd7(nx,ny,nz,dx,dy,dz);

[n_cold_calcdata] = update_np(beta,beta_p,ijkp,reshape(allx_solarwind,[length(allx_solarwind),3]),Ni_tot,wght,...
    nx,ny,nz,...
    dx_grid,dy_grid,dz_grid,...
    qx,qy,qz,...
    dx,dy,dz);
[up_cold_calcdata] = update_up(beta,beta_p,ijkp,n_cold_calcdata,reshape(allv_solarwind,[length(allx_solarwind),3]),Ni_tot,wght,nx,ny,nz,qx,qy,qz);
% ncalc = reshape(np(:,2,:),[nx nz]);
%  uxcalc = reshape(up_mixed_data(:,2,:,1),[nx nz]);
%  uycalc = reshape(up_mixed_data(:,2,:,2),[nx nz]);
%  uzcalc = reshape(up_mixed_data(:,2,:,3),[nx nz]);
...
procnum=24;
nf_init = 5e15;
Ni_tot = length(allx_foreshock);
ijkp=zeros(Ni_tot,3);
beta_particle = 1.0;
beta_p = beta_particle * ones(Ni_tot,1);
Ni_tot_sys = floor((nx-2)*(ny-2)*(nz-2)*(ppc/procnum))*procnum;

[wght,ijkp] = get_interp_weights(ijkp,reshape(allx_foreshock,[length(allx_foreshock),3]),Ni_tot,qx,qy,qz,nx,ny,nz,dx,dy,dz);
[beta] = get_beta(Ni_tot_sys,nf_init,qx,qy,qz,nx,ny,nz);
[dx_grid,dy_grid,dz_grid,qx,qy,qz] = grd7(nx,ny,nz,dx,dy,dz);

[n_mixed_calcdata] = update_np(beta,beta_p,ijkp,reshape(allx_foreshock,[length(allx_foreshock),3]),Ni_tot,wght,...
    nx,ny,nz,...
    dx_grid,dy_grid,dz_grid,...
    qx,qy,qz,...
    dx,dy,dz);
[up_mixed_calcdata] = update_up(beta,beta_p,ijkp,n_mixed_calcdata,reshape(allv_foreshock,[length(allx_foreshock),3]),Ni_tot,wght,nx,ny,nz,qx,qy,qz);
% % ncalc = reshape(np(:,2,:),[nx nz]);
%  uxcalc = reshape(up_mixed_data(:,2,:,1),[nx nz]);
%  uycalc = reshape(up_mixed_data(:,2,:,2),[nx nz]);
%  uzcalc = reshape(up_mixed_data(:,2,:,3),[nx nz]);
...
zrange = [150-51, 300+150+51];

moverq=1.0378e-8;
% fileName = strcat('/home/andrew/Documents/output/2d_Hybrid_CurrentDecomp_',RunNumber,'_',string(xcut));
...
[n_cold_data_interp0] = interpolate_Data(ndata,1,1,nx,nz,X,Z,X2,Z2);
% n_cold_data_interp0 = reshape(n_cold_data(:,2,:),[nx nz])';
...
[upx_cold_data_interp0] = ((interpolate_Data(udata,1,1,nx,nz,X,Z,X2,Z2)));
[upy_cold_data_interp0] = ((interpolate_Data(udata,2,1,nx,nz,X,Z,X2,Z2)));
[upz_cold_data_interp0] = ((interpolate_Data(udata,3,1,nx,nz,X,Z,X2,Z2)));
...
%     [n_mixed_data_interp] = (interpolate_Data(n_mixed_data,1,i,nx,nz,X,Z,X2,Z2));
    n_cold_data_interp = reshape(n_cold_calcdata(:,2,:),[nx nz])';
    n_mixed_data_interp = reshape(n_mixed_calcdata(:,2,:),[nx nz])';
...
%     upz_cold_data_interp = ((interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2)));
    upx_cold_data_interp = reshape(up_cold_calcdata(:,2,:,1),[nx nz])';
    upy_cold_data_interp = reshape(up_cold_calcdata(:,2,:,2),[nx nz])';
    upz_cold_data_interp = reshape(up_cold_calcdata(:,2,:,3),[nx nz])';
...
%     upz_mixed_data_interp = (interpolate_Data(up_mixed_data,3,i,nx,nz,X,Z,X2,Z2));
    upx_mixed_data_interp = reshape(up_mixed_calcdata(:,2,:,1),[nx nz])';
    upy_mixed_data_interp = reshape(up_mixed_calcdata(:,2,:,2),[nx nz])';
    upz_mixed_data_interp = reshape(up_mixed_calcdata(:,2,:,3),[nx nz])';
...
%JdotE
    ...
    Eswx=Exdata_interp/E0 + (upy_cold_data_interp0.*Bzdata_interp-upz_cold_data_interp0.*Bydata_interp)/(va*B0);%-Eswx0;
    Eswy=Eydata_interp/E0 + (upz_cold_data_interp0.*Bxdata_interp-upx_cold_data_interp0.*Bzdata_interp)/(va*B0);%-Eswy0;
    Eswz=Ezdata_interp/E0 + (upx_cold_data_interp0.*Bydata_interp-upy_cold_data_interp0.*Bxdata_interp)/(va*B0);%-Eswz0;
    
    
    JdotE_cold = delta_Jx_cold.*Eswx + delta_Jy_cold.*Eswy + delta_Jz_cold.*Eswz;
    JdotE_mixed = delta_Jx_mixed.*Eswx + delta_Jy_mixed.*Eswy + delta_Jz_mixed.*Eswz;
    ...
    hold off
    yyaxis left
    plot(smooth((n_cold_data_interp_plot(2:end-1,xcut)-0*n_cold_data_interp0_plot(2:end-1,xcut))./n0),'b','linewidth',1)
    hold on
%     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    yyaxis right
    hold on
    plot(smooth((n_mixed_data_interp_plot(2:end-1,xcut))./n0),'r','linewidth',1)
%     plot(smooth((n_mixed_data_interp_plot(2:end-1,xcut))./n0),'r','linewidth',1)
...
%     ylim([0 0.24])
    ...
    nlegend = legend('sw','fs','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    plot(smooth(delta_Bxdata_interp(:,xcut)./B0),'m','linewidth',1)
    hold on
    ...
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    legend('Bx','By','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
...
%     legend('x','y','z','Location','northeast','Orientation','horizontal'); legend('boxoff')
    ...
    delta_upperpx_cold =imgaussfilt(delta_upperpx_cold,filterNumber5)./va;
    delta_upperpx_mixed=imgaussfilt(delta_upperpx_mixed,filterNumber5)./va;
    delta_ueperpx = imgaussfilt(delta_ueperpx,filterNumber5);
    
    delta_upparax_cold =imgaussfilt(delta_upparax_cold,filterNumber5)./va;
    delta_upparax_mixed=imgaussfilt(delta_upparax_mixed,filterNumber5)./va;
    delta_ueparax = imgaussfilt(delta_ueparax,filterNumber5);
    
%     axes(ha(axes_count));axes_count=axes_count+1;
%     yyaxis left
%     plot(smooth(delta_upperpx_cold(:,xcut)),'b','linewidth',1)
%     hold on
%     plot(smooth((delta_ueperpx(:,xcut))),'g--','linewidth',1)
%     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
%     yyaxis right
%     plot(smooth((delta_upperpx_mixed(:,xcut))),'r','linewidth',1)
%     %     ylim(ExBrightlimits)
%     %     yyaxis left
%     %     ylim(ExBleftlimits)
%     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
...
%     legend('\Deltau_{sw}','\Deltau_{e}','u_{fs}','Location','northeast','Orientation','horizontal'); legend('boxoff')
    ...
    delta_electron_parax_cold = imgaussfilt(delta_electron_parax_cold,filterNumber);
    delta_cold_parax = imgaussfilt(delta_cold_parax,filterNumber);
    delta_electron_parax_mixed = imgaussfilt(delta_electron_parax_mixed,filterNumber);
    delta_mixed_parax = imgaussfilt(delta_mixed_parax,filterNumber);
...
%     set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
%     
    Jperpx_cold = imgaussfilt(Jperpx_cold,filterNumber5);
    Jperpx_mixed = imgaussfilt(Jperpx_mixed,filterNumber5);
    
    Jparax_cold = imgaussfilt(Jparax_cold,filterNumber5);
    Jparax_mixed = imgaussfilt(Jparax_mixed,filterNumber5);
    ...
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    if del==1
        ylabel({'\DeltaJ_{\perpx}/J0'},'FontSize',14)
    else
        ylabel({'J_{\perpx}/J0'},'FontSize',14)
    end
    xlim(zrange)
    line1 = yline(0,'k:','LineWidth',1.5);
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    AAlegend = legend('sw','fs','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    plot(smooth((delta_Jperpx_minus_delta_electron_perpx(2:end-1,xcut))),'linewidth',1,'color',[0.75 0 0.75])
    hold on
    plot(smooth(Jperpx_tot_woJd(:,xcut)),'k','linewidth',1)    
    hold off
    ...
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    Blegend = legend('sw+fs','\nablaxB','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
...
%     Jperpy_mixed = imgaussfilt(Jperpy_mixed,filterNumber);    
    ...
    delta_upperpy_cold =imgaussfilt(delta_upperpy_cold,filterNumber5)./va;
    delta_upperpy_mixed=imgaussfilt(delta_upperpy_mixed,filterNumber5)./va;
    delta_ueperpy = imgaussfilt(delta_ueperpy,filterNumber5);
    
    delta_upparay_cold =imgaussfilt(delta_upparay_cold,filterNumber5)./va;
    delta_upparay_mixed=imgaussfilt(delta_upparay_mixed,filterNumber5)./va;
    delta_ueparay = imgaussfilt(delta_ueparay,filterNumber5);
...
%     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
%     yyaxis right
%     plot(smooth((delta_upperpy_mixed(:,xcut))),'r','linewidth',1)
%     
%     
% %     ylim(ExBrightlimits)
% %     yyaxis left
...
%     legend('\Deltau_{sw}','\Deltau_{e}','u_{fs}','Location','northeast','Orientation','horizontal'); legend('boxoff')
    ...
    Jperpy_cold = imgaussfilt(Jperpy_cold,filterNumber5);
    Jperpy_mixed = imgaussfilt(Jperpy_mixed,filterNumber5);
    
    Jparay_cold = imgaussfilt(Jparay_cold,filterNumber5);
    Jparay_mixed = imgaussfilt(Jparay_mixed,filterNumber5);
    ...
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    if del==1 
       ylabel({'\DeltaJ_{\perpy}/J_0'},'FontSize',14) 
    else
    ylabel({'J\perpy/J_0'},'FontSize',14)
    end
    xlim(zrange)
    line3 = yline(0,'k:','LineWidth',1.5);

    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    CClegend = legend('sw','fs','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
    ...
    plot(smooth((delta_Jperpy_minus_delta_electron_perpy(2:end-1,xcut))),'linewidth',1,'color',[0.75 0 0.75])
    hold on
    plot(smooth(Jperpy_tot_woJd(:,xcut)),'k','linewidth',1)
    ...
    Dlegend = legend('sw+fs','\nablaxB','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
   
    
    JdotE_cold = imgaussfilt(JdotE_cold,filterNumber4);
    JdotE_mixed = imgaussfilt(JdotE_mixed,filterNumber4);
    
    axes(ha(axes_count));axes_count=axes_count+1; %#ok<*LAXES>
    yyaxis left
    plot(smooth(JdotE_cold(:,xcut)),'b','linewidth',1)
    hold on
    
    yyaxis right
    plot(smooth((JdotE_mixed(2:end-1,xcut))),'r','linewidth',1)
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    ...
    %     end
    hold off
    ylabel({'\DeltaJ \cdot \DeltaE/(J0E0)'},'FontSize',14)
    xlim(zrange)
    yyaxis right
    line4 =    yline(0,'k:','LineWidth',1.5);
    yyaxis left
    ...
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5); xlabel({'Z','[\lambda_i]'},'FontSize',14)
    Dlegend = legend('sw','fs','Location','northeast','Orientation','horizontal','fontsize',12); legend('boxoff')
   
    
    if min(JdotE_mixed(2:end-1,xcut)) < JdotE_mixed_minimum
        JdotE_mixed_minimum = min(JdotE_mixed(2:end-1,xcut))
    end
