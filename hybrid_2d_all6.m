...
fig = figure('Position',[ 1 1 1800 800]);
...
startstreamX = 1:1:nx;
moverq=1.044e-8;
v = VideoWriter('2d_HybridForeshockDD');
v.FrameRate= 1;
open(v);
...
timeTimeSeries = [];
for i=1:nt
    h = sgtitle(sprintf('time = %2.2f',output*dt*double(i)));
    subplot(5,3,1)
colormap(A)
...
%     imagesc((log(ndata_interp(nz_min:nz_max,:))));  hold on;
    ...
    subplot(5,3,3)
    %Time Series for Densitty
%         densityTimeSeries = [densityTimeSeries, ndata_interp(120,90)];
%         timeTimeSeries = [timeTimeSeries,output*dt*i];
%         plot(timeTimeSeries,densityTimeSeries./(1e15))
%         title('Ion Density at (90,120)')
...
%     imagesc((log(ndata_interp(nz_min:nz_max,:))));  hold on;
    imagesc((log10(nFSdata_interp(nz_min:nz_max,nx_min:nx_max))));  hold on;
    ...
    [Bxdata_interp] = interpolate_Data(Bdata,1,i,nx,nz,X,Z,X2,Z2).*moverq;
    [Bydata_interp] = interpolate_Data(Bdata,2,i,nx,nz,X,Z,X2,Z2).*moverq;
    [Bzdata_interp] = interpolate_Data(Bdata,3,i,nx,nz,X,Z,X2,Z2).*moverq;
...
function [data_interp] = interpolate_Data(Data,component,t,nx,nz,X,Z,X2,Z2)
    if component ~= 0
        data_interp=reshape(Data(t,:,2,:,component),[nx,nz]);
        %data_interp=interp2(X,Z,data_interp,X2,Z2,'linear');
        data_interp=data_interp';
    else
        data_interp=reshape(Data(t,:,2,:),[nx,nz]);
        %data_interp=interp2(X,Z,data_interp,X2,Z2,'linear');
        data_interp=data_interp';
    end
...
function [qx,qy,qz,nt,nx,ny,nz,va] = read_Coordinates()
    int='int32';real='float32';
    %%%%%%%%%%%%%%%%%%%READ PARA.DAT%%%%%%%%%%%%%%%%%%%%%%%
    [fid,~]=fopen('para.dat','r','n');
    fread(fid,1,int);
...
function [f2] = read_Plasma(Plasma,nt,nx,ny,nz)
    %%%%%%%%%%%%%%%%%%%%%%%%%READ CHOSEN FILE%%%%%%%%%%%%%%%%%%%%%%%%%%
    int='int32';real='float32';
    switch Plasma
        case 'n'
