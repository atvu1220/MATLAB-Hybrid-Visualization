...
del = 1;
%%
% figure; plot(delta_electron_perpx_cold(:,xcut)+delta_electron_perpx_mixed(:,xcut)); hold on; plot(delta_electron_perpx(:,xcut));
% figure; plot(delta_electron_perpy_cold(:,xcut)+delta_electron_perpy_mixed(:,xcut)); hold on; plot(delta_electron_perpy(:,xcut));
...
moverq=1.0378e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_Jperp_minusElectrons8m_',RunNumber,'_',string(xcut));
...
[Exdata_interp0] = imgaussfilt((interpolate_Data(Edata,1,1,nx,nz,X,Z,X2,Z2)),filterNumber);
[Eydata_interp0] = imgaussfilt((interpolate_Data(Edata,2,1,nx,nz,X,Z,X2,Z2)),filterNumber);
[Eydata_interp0_y1] = imgaussfilt((interpolate_Data_y(Edata,2,1,nx,nz,X,Z,X2,Z2)),filterNumber);
[Ezdata_interp0] = imgaussfilt((interpolate_Data(Edata,3,1,nx,nz,X,Z,X2,Z2)),filterNumber);

[gradPexdata_interp0] = imgaussfilt((interpolate_Data(gradPdata,1,1,nx,nz,X,Z,X2,Z2)),filterNumber);
[gradPeydata_interp0] = imgaussfilt((interpolate_Data(gradPdata,2,1,nx,nz,X,Z,X2,Z2)),filterNumber);
[gradPezdata_interp0] = imgaussfilt((interpolate_Data(gradPdata,3,1,nx,nz,X,Z,X2,Z2)),filterNumber);
...
for i=1:stepInterval:nt
    ...
    [tempx_cold_data_interp] = imgaussfilt(interpolate_Data(tpcolddata,1,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempx_cold_data_interp(end-3:end,:) = 0;
    [tempy_cold_data_interp] = imgaussfilt(interpolate_Data(tpcolddata,2,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempy_cold_data_interp(end-3:end,:) = 0;
    [tempz_cold_data_interp] = imgaussfilt(interpolate_Data(tpcolddata,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempz_cold_data_interp(end-3:end,:) = 0;
    [tempx_cold_data_interp_y1] = imgaussfilt(interpolate_Data_y(tpcolddata,1,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempx_cold_data_interp_y1(end-3:end,:) = 0;
    [tempy_cold_data_interp_y1] = imgaussfilt(interpolate_Data_y(tpcolddata,2,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempy_cold_data_interp_y1(end-3:end,:) = 0;
    [tempz_cold_data_interp_y1] = imgaussfilt(interpolate_Data_y(tpcolddata,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempz_cold_data_interp_y1(end-3:end,:) = 0;
    
    [tempx_mixed_data_interp] = imgaussfilt(interpolate_Data(tpmixeddata,1,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempx_mixed_data_interp(end-3:end,:) = 0;
    [tempy_mixed_data_interp] = imgaussfilt(interpolate_Data(tpmixeddata,2,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempy_mixed_data_interp(end-3:end,:) = 0;
    [tempz_mixed_data_interp] = imgaussfilt(interpolate_Data(tpmixeddata,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempz_mixed_data_interp(end-3:end,:) = 0;
    [tempx_mixed_data_interp_y1] = imgaussfilt(interpolate_Data_y(tpmixeddata,1,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempx_mixed_data_interp_y1(end-3:end,:) = 0;
    [tempy_mixed_data_interp_y1] = imgaussfilt(interpolate_Data_y(tpmixeddata,2,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempy_mixed_data_interp_y1(end-3:end,:) = 0;
    [tempz_mixed_data_interp_y1] = imgaussfilt(interpolate_Data_y(tpmixeddata,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    tempz_mixed_data_interp_y1(end-3:end,:) = 0;
    
    
    %Electric Fields
    [Exdata_interp] = imgaussfilt((interpolate_Data(Edata,1,i,nx,nz,X,Z,X2,Z2)),filterNumber);
    [Eydata_interp] = imgaussfilt((interpolate_Data(Edata,2,i,nx,nz,X,Z,X2,Z2)),filterNumber);
    [Eydata_interp_y1] = imgaussfilt((interpolate_Data_y(Edata,2,i,nx,nz,X,Z,X2,Z2)),filterNumber);
    [Ezdata_interp] = imgaussfilt((interpolate_Data(Edata,3,i,nx,nz,X,Z,X2,Z2)),filterNumber);
    ...
    delta_Exdata_interp = (Exdata_interp-Exdata_interp0)./E0;
    delta_Eydata_interp = (Eydata_interp-Eydata_interp0)./E0;
    delta_Eydata_interp_y1 = (Eydata_interp_y1-Eydata_interp0_y1)./E0;
    delta_Ezdata_interp = (Ezdata_interp-Ezdata_interp0)./E0;
    ...
    divdeltaEdata_interp = imgaussfilt((partialdeltaExdata_interp + partialdeltaEydata_interp + partialdeltaEzdata_interp)./(E0/lambda_i),filterNumber);
    
    
%     [gradPexdata_interp] = imgaussfilt((interpolate_Data(gradPdata,1,i,nx,nz,X,Z,X2,Z2)),filterNumber);
%     [gradPeydata_interp] = imgaussfilt((interpolate_Data(gradPdata,2,i,nx,nz,X,Z,X2,Z2)),filterNumber);
%     [gradPezdata_interp] = imgaussfilt((interpolate_Data(gradPdata,3,i,nx,nz,X,Z,X2,Z2)),filterNumber);
%     
%     delta_gradPexdata_interp = (gradPexdata_interp-gradPexdata_interp0)./E0;
%     delta_gradPeydata_interp = (gradPeydata_interp-gradPeydata_interp0)./E0;
%     delta_gradPezdata_interp = (gradPezdata_interp-gradPezdata_interp0)./E0;
    ...
    delta_Bxdata_interp = Bxdata_interp - Bxdata_interp0;
    delta_Bydata_interp = Bydata_interp - Bydata_interp0;
    delta_Bzdata_interp = Bzdata_interp - Bzdata_interp0;
    ...
    %Delta ExB
    delta_ExBx = ExBx-del*ExBx0;
    delta_ExBy = ExBy-del*ExBy0;
    delta_ExBz = ExBz-del*ExBz0;
...
%     pex_mixed0 = 0.12.*n0 .* uex_tot0;
%     pey_mixed0 = 0.12.*n0 .* uey_tot0;
%     pez_mixed0 = 0.12.*n0 .* uez_tot0;
...
%     [electron_parax_mixed0,electron_paray_mixed0,electron_paraz_mixed0,electron_perpx_mixed0,electron_perpy_mixed0,electron_perpz_mixed0] = calculate_componentsVector(Bxdata_interp,Bydata_interp,Bzdata_interp,pex_mixed0,pey_mixed0,pez_mixed0);
%     
%     delta_electron_parax_mixed = (electron_parax_mixed-0*electron_parax_mixed0).*10^3 * 10^-9.*-q./nv0;
%     delta_electron_paray_mixed = (electron_paray_mixed-0*electron_paray_mixed0).*10^3 * 10^-9.*-q./nv0;
%     delta_electron_paraz_mixed = (electron_paraz_mixed-0*electron_paraz_mixed0).*10^3 * 10^-9.*-q./nv0;
%     delta_electron_perpx_mixed = (electron_perpx_mixed-0*electron_perpx_mixed0).*10^3 * 10^-9.*-q./nv0;
%     delta_electron_perpy_mixed = (electron_perpy_mixed-0*electron_perpy_mixed0).*10^3 * 10^-9.*-q./nv0;
%     delta_electron_perpz_mixed = (electron_perpz_mixed-0*electron_perpz_mixed0).*10^3 * 10^-9.*-q./nv0;
    ...
    [~,~,~,ueperpx0,ueperpy0,ueperpz0] = calculate_componentsVector(Bxdata_interp,Bydata_interp,Bzdata_interp,uex_tot0,uey_tot0,uez_tot0);
    
%     delta_ueperpx = ndata_interp.*ueperpx - del*ndata_interp0.*ueperpx0;
%     delta_ueperpy = ndata_interp.*ueperpy - del*ndata_interp0.*ueperpy0;
%     delta_ueperpz = ndata_interp.*ueperpz - del*ndata_interp0.*ueperpz0;
    
    delta_ueperpx = ueperpx - del*ueperpx0;
    delta_ueperpy = ueperpy - del*ueperpy0;
    delta_ueperpz = ueperpz - del*ueperpz0;
    ...
    %Diamagnetic Current Calculations//Gradient Pressure
...
%     Pt_cold = n_cold_data_interp.*(tempperp_cold_data_interp).*10^-9.*q;
...
%     Pt_cold_y1 = 2*n_cold_data_interp_y1.*(tempperp_cold_data_interp_y1).*10^-9.*q;
...
%     Pt_mixed = n_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold)).*(tempperp_mixed_data_interp).*((n_mixed_data_interp./n0 > nFS_threshold)).*10^-9.*q;
%     
...
%     Pt_mixed_y1 = 2*n_mixed_data_interp_y1.*((n_mixed_data_interp_y1./n0 > nFS_threshold)).*(tempperp_mixed_data_interp_y1).*((n_mixed_data_interp_y1./n0 > nFS_threshold)).*10^-9.*q;
...
%     Jperpy_tot_woJd = delta_cold_perpy(2:end-1,2:end-1) + delta_mixed_perpy(2:end-1,2:end-1);% + delta_electron_perpy(2:end-1,2:end-1);
    ...
    hold off
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    ylabel({'\DeltaB/B0'},'FontSize',14)
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    AAlegend = legend('Bx','By','Location','northeast','Orientation','horizontal'); legend('boxoff')
    %     Clegend = legend('Jperptot wo. gradPe','Jperptot w. gradPe','\DeltaJperp','Location','northwest','Orientation','horizontal'); legend('boxoff')
    
    
    axes(ha(axes_count));axes_count=axes_count+1;
    yyaxis left
    plot(smooth(partialdeltaExdata_interp(:,xcut)),'b','linewidth',1)
    hold on
    plot(smooth((partialdeltaEydata_interp(:,xcut))),'g','linewidth',1)
    yyaxis right
    plot(smooth((partialdeltaEzdata_interp(:,xcut))),'r','linewidth',1)
    hold on
    
    plot(smooth((divdeltaEdata_interp(:,xcut))),'k--','linewidth',1)
    hold off
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    yyaxis left
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    ...
    if del == 1
        ylabel({'\DeltadivE/E0'},'FontSize',14)
    else
        ylabel({'divE/E0'},'FontSize',14)
    end
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    AAlegend = legend('Ex','Ey','Ez','divE','Location','northeast','Orientation','horizontal'); legend('boxoff')
    %     Clegend = legend('Jperptot wo. gradPe','Jperptot w. gradPe','\DeltaJperp','Location','northwest','Orientation','horizontal'); legend('boxoff')
    ...
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    AAlegend = legend('Ex','Ez','Ey','Location','northeast','Orientation','horizontal'); legend('boxoff')
    
    %     axes(ha(axes_count));axes_count=axes_count+1;
    % %     yyaxis left
    %     plot(smooth(delta_gradPexdata_interp(:,xcut)),'b','linewidth',1)
    %     hold on
    % %     yyaxis right
    %     plot(smooth((delta_gradPezdata_interp(:,xcut))),'r','linewidth',1)
    % %     hold on
    %     plot(smooth((delta_gradPeydata_interp(:,xcut))),'g','linewidth',1)
    %     hold off
    % %     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    % %     yyaxis left
    % %     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    %     ylim([-0.05 0.05])
    %     ylabel({'\DeltaGradPe/E0'},'FontSize',14)
    %     xlim(zrange)
    %     set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    %     AAlegend = legend('gradPx','gradPz','gradPy','Location','northeast','Orientation','horizontal'); legend('boxoff')
    
    
    
    axes(ha(axes_count));axes_count=axes_count+1;
    yyaxis left
    plot(smooth(delta_ExBx(:,xcut)./(va.*B0)),'b','linewidth',1)
    hold on
    yyaxis right
    plot(smooth((delta_ExBz(:,xcut))./(va.*B0)),'r','linewidth',1)
    hold on
    plot(smooth((delta_ExBy(:,xcut))./(va.*B0)),'g','linewidth',1)
    hold off
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    yyaxis left
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    ...
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    AAlegend = legend('ExBx','ExBz','ExBy','Location','northeast','Orientation','horizontal'); legend('boxoff')
    
    axes(ha(axes_count));axes_count=axes_count+1;
    yyaxis left
    plot(smooth(delta_ueperpx(:,xcut)./va),'b','linewidth',1)
    hold on
    yyaxis right
    plot(smooth((delta_ueperpz(:,xcut))./va),'r','linewidth',1)
    hold on
    plot(smooth((delta_ueperpy(:,xcut))./va),'g','linewidth',1)
    hold off
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    yyaxis left
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    %     ylim([-0.05 0.05])
    if del == 1
    ylabel({'\Deltau_e/va'},'FontSize',14)
    else
    ...
    xlim(zrange)
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    AAlegend = legend('u_eperpx','u_eperpz','u_eperpy','Location','northeast','Orientation','horizontal'); legend('boxoff')
    ...
    %     hold on
    %     plot(smooth(jDx_cold(:,xcut)),'b--','linewidth',1)
    %     hold off
    %     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    %
    ...
    %     hold on
    %     plot(smooth(jDx_mixed(:,xcut)),'r--','linewidth',1)
    %     hold off
    %     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    %
    ...
    %     xlim(zrange)
    %     AAAlegend = legend('\Deltanqv_{sw}','jD_{sw}','nqv_{fs}','jD_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    %     set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    ...
    hold on
    %     plot(smooth(jDx_cold(:,xcut)),'b--','linewidth',1)
    plot(smooth(delta_electron_perpx_cold(2:end-1,xcut)+delta_electron_perpx_mixed(2:end-1,xcut)),'k--','linewidth',1)
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    %     plot(smooth(jDx_cold(:,xcut)),'linewidth',1)
    %     plot(smooth(delta_electron_perpx(2:end-1,xcut)),'linewidth',1)
    %     yyaxis right
    ...
    xlim(zrange)
    Alegend = legend('\Delta(nqu)_{sw}','\Delta(nqu)_{e}','(nqu)_{fs}','Location','northeast','Orientation','horizontal'); legend('boxoff')
    ...
    %     hold on
    %      plot(smooth(jDy_cold(:,xcut)),'b--','linewidth',1)
    %     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    %     hold on;
    ...
    %     hold on
    %     plot(smooth(jDy_mixed(:,xcut)),'r--','linewidth',1)
    %
    %     ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    ...
    %     xlim(zrange)
    %     CCClegend = legend('\Deltanqv_{sw}','jD_{sw}','nqv_{fs}','jD_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    ...
    hold on
    %     plot(smooth(jDy_cold(:,xcut)),'b--','linewidth',1)
    plot(smooth(delta_electron_perpy_cold(2:end-1,xcut)+delta_electron_perpy_mixed(2:end-1,xcut)),'k--','linewidth',1)
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    %     hold on;
    %     plot(smooth(jDy_cold(:,xcut)),'linewidth',1)
    %     plot(smooth(delta_electron_perpy(2:end-1,xcut)),'linewidth',1)
    ...
    xlim(zrange)
    Clegend = legend('\Delta(nqu)_{sw}','\Delta(nqu)_{e}','(nqu)_{fs}','Location','northeast','Orientation','horizontal'); legend('boxoff')
    %     Blegend = legend('\Deltanqv_{sw}','\Deltanqv_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')'\Deltanqv_{sw}','jD_{sw}''\Deltanqv_{fs}','jD_{fs}'
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
