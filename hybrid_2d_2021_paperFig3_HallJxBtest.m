...
% filterNumbern = 0.0001;
% filterNumber = 0.0001;%0.1;
% filterNumber2 = 0.0001;%0.01;
% filterNumber3 = 0.0001; %Particle J
% filterNumber4=0.0001; %J current and B
% filterNumber5=0.0001; %u and Ji-Je
...
%     yyaxis right
    ...
    Eprimex=Exdata_interp./E0 + (upy_cold_data_interp0.*Bzdata_interp-upz_cold_data_interp0.*Bydata_interp)./va./B0;
    Eprimey=Eydata_interp./E0 + (upz_cold_data_interp0.*Bxdata_interp-upx_cold_data_interp0.*Bzdata_interp)./va./B0;
    Eprimez=Ezdata_interp./E0 + (upx_cold_data_interp0.*Bydata_interp-upy_cold_data_interp0.*Bxdata_interp)./va./B0;
    axes(ha(axes_count));axes_count=axes_count+1;
...
%     JtotxBx = (upy_data_interp-uey_tot).*Bzdata_interp./B0 - (upz_data_interp-uez_tot).*Bydata_interp./B0;
%     JtotxBy = (upz_data_interp-uez_tot).*Bxdata_interp./B0 - (upx_data_interp-uex_tot).*Bzdata_interp./B0;
%     JtotxBz = (upx_data_interp-uex_tot).*Bydata_interp./B0 - (upy_data_interp-uey_tot).*Bxdata_interp./B0;
...
%     %VixB and VexB
     axes(ha(axes_count));axes_count=axes_count+1;
% %     yyaxis left
%     UixBx =    (delta_upy)./va.*delta_Bzdata_interp./B0 - (delta_upz)./va.*delta_Bydata_interp./B0;
%     UixBy = -( (delta_upx)./va.*delta_Bzdata_interp./B0 - (delta_upz)./va.*delta_Bxdata_interp./B0 );
%     UixBz =    (delta_upx)./va.*delta_Bydata_interp./B0 - (delta_upy)./va.*delta_Bxdata_interp./B0;
%     
    UixBx = -   (upy_data_interp-upy_cold_data_interp0)./va.*Bzdata_interp./B0 - (upz_data_interp-upz_cold_data_interp0)./va.*Bydata_interp./B0;
    UixBy = +(  (upx_data_interp-upx_cold_data_interp0)./va.*Bzdata_interp./B0 - (upz_data_interp-upz_cold_data_interp0)./va.*Bxdata_interp./B0 );
    UixBz = -   (upx_data_interp-upx_cold_data_interp0)./va.*Bydata_interp./B0 - (upy_data_interp-upz_cold_data_interp0)./va.*Bxdata_interp./B0;
    %
%     
...
%     yyaxis left
    ...
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5)
        ...
        EEx = JtotxBx+UixBx-upx_data_interp0./va;
        EEy = JtotxBy+UixBy-upy_data_interp0./va;
        EEz = JtotxBz+UixBz-upz_data_interp0./va;
