...
fig = figure('Position',[ 1 1 300+5*250 3*250]);
outputFolder = '184';
...
ylimits = [151 249];
xlimits = [61 159];
xlimits = [110-49 110+49]
filterNumber = 1;

moverq=1.0378e-8;
q=1.6*10^(-19);
alpha = 1.9373612*10^(-20);
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_PaperFig2_',RunNumber);
...
%180
particlesthatcrossTD = max(f2(:,:,3)./lambda_i);
[maxTimeSteps,particleIndex] = maxk(particlesthatcrossTD,particlesaboveTD);
nTrackParticles = [particleIndex(2:4), 15+132:264:nTestParticles];

%179
% particlesthatcrossTD = max(f2(:,:,3)./lambda_i);
% particlesthatcrossTD(particlesthatcrossTD > nz/2) = 0;
% [maxTimeSteps,particleIndex] = maxk(particlesthatcrossTD,particlesaboveTD);
% nTrackParticles = [particleIndex(3), 15+132:1*264:nTestParticles];

for i=40:stepInterval:nt
    i
...
%     caxis([-max(abs(caxis-2*FS_Ma))-2*FS_Ma max(abs(caxis-2*FS_Ma))-2*FS_Ma])
    ...
    %     clear upz_mixed_data_interp
    colorbar; title('FS \Deltavz/va','FontSize', 14)
    %shading interp
    %      caxis([-3 3])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2);ylim(ylimits);xlim(xlimits)
    ...
    upx_mixed_data_interp0 = va.*0.12*n0.*upx_mixed_bulk.*((n_mixed_data_interp./n0 > nFS_threshold));
    upy_mixed_data_interp0 = va.*0.12*n0.*upy_mixed_bulk.*((n_mixed_data_interp./n0 > nFS_threshold));
    upz_mixed_data_interp0 = va.*0.12*n0.*upz_mixed_bulk.*((n_mixed_data_interp./n0 > nFS_threshold));
