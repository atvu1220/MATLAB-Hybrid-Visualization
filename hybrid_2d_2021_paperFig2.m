...
ylimits = [200 400];
ddthickness = 24;
ylimits = [(nz-2)/2 - 1*ddthickness (nz-2)/2 + 1*ddthickness];
xlimits = [2 nx-4];
xlimits = [ 31 79];
filterNumber = 1;
...
% Tdata_interp = imgaussfilt(Tdata_interp,1);
% t0 = mean(Tdata_interp(floor(nz-nz/4):nz-1,2:end-4),'all');
% t0 = imgaussfilt(t0,1);

[n_cold_data_interp] = interpolate_Data(n_cold_data,1,1,nx,nz,X,Z,X2,Z2);
n_cold_data_interp = imgaussfilt(n_cold_data_interp,1);
n0 = mean(n_cold_data_interp(3:nz-3,2:end-4),'all');
n0 = imgaussfilt(n0,1);

[Bxdata_interp] = (interpolate_Data(Bdata,1,1,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9);
[Bydata_interp] = (interpolate_Data(Bdata,2,1,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9);
[Bzdata_interp] = (interpolate_Data(Bdata,3,1,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9);


[upx_cold_data_interp0] = n_cold_data_interp.*(interpolate_Data(up_cold_data,1,1,nx,nz,X,Z,X2,Z2))./va./n0;
[upy_cold_data_interp0] = n_cold_data_interp.*(interpolate_Data(up_cold_data,2,1,nx,nz,X,Z,X2,Z2))./va./n0;
[upz_cold_data_interp0] = n_cold_data_interp.*(interpolate_Data(up_cold_data,3,1,nx,nz,X,Z,X2,Z2))./va./n0;

[upx_cold_data_interp] = n_cold_data_interp.*(interpolate_Data(up_cold_data,1,1,nx,nz,X,Z,X2,Z2))./va./n0;
[upy_cold_data_interp] = n_cold_data_interp.*(interpolate_Data(up_cold_data,2,1,nx,nz,X,Z,X2,Z2))./va./n0;
[upz_cold_data_interp] = n_cold_data_interp.*(interpolate_Data(up_cold_data,3,1,nx,nz,X,Z,X2,Z2))./va./n0;
[uparacold0,uperpcold0] = calculate_components(Bxdata_interp,Bydata_interp,Bzdata_interp,upx_cold_data_interp,upy_cold_data_interp,upz_cold_data_interp);

[Jxdata_interp] = interpolate_Data(Jdata,1,1,nx,nz,X,Z,X2,Z2);%.*ndata_interp.*alpha;
Jxdata_interp = imgaussfilt(Jxdata_interp,filterNumber);
[Jydata_interp] = interpolate_Data(Jdata,2,1,nx,nz,X,Z,X2,Z2);%.*ndata_interp.*alpha;
Jydata_interp = imgaussfilt(Jydata_interp,filterNumber);
[Jzdata_interp] = interpolate_Data(Jdata,3,1,nx,nz,X,Z,X2,Z2);%.*ndata_interp.*alpha;
Jzdata_interp = imgaussfilt(Jzdata_interp,filterNumber);
[Jpara0,Jperp0] = calculate_components(Bxdata_interp,Bydata_interp,Bzdata_interp,Jxdata_interp,Jydata_interp,Jzdata_interp);
...
nTrackParticles = 2:31:nTestParticles;

particlesthatcrossTD = sum(f2(:,:,3)./lambda_i > (nz-2)/2 );
[maxTimeSteps,particleIndex] = maxk(particlesthatcrossTD,particlesaboveTD);
nTestParticles = length(nonzeros(f2(end,:,1)));
 nTrackParticles = [particleIndex 15:78:nTestParticles];
...
%     plot7=subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(1));
    [upx_cold_data_interp] = n_cold_data_interp.*interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2)./va - upx_cold_data_interp0;
    %         upx_mixed_data_interp=upx_mixed_data_interp.*((n_mixed_data_interp > nFS_threshold))./va;
    %clear up_mixed_data_interp
    pcolor(upx_cold_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upx_cold_data_interp
    colorbar; title('SW \Deltanvx','FontSize', 14)
    %shading interp
%     caxis([8 16])
...
%     plot8=subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(2));
    [upy_cold_data_interp] = n_cold_data_interp.*interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2)./va - upy_cold_data_interp0;
    %         upy_cold_data_interp=upy_cold_data_interp.*((n_mixed_data_interp > nFS_threshold))./va;
    %clear up_mixed_data_interp
    pcolor(upy_cold_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upy_cold_data_interp
    colorbar; title('SW \Deltanvy','FontSize', 14)
    %shading interp
%     caxis([-2 2])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
...
%     plot9=subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(3));
    [upz_cold_data_interp] = n_cold_data_interp.*interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2)./va - upz_cold_data_interp0;
...
%     clear upz_mixed_data_interp
    colorbar; title('SW \Deltanvz','FontSize', 14)
    %shading interp
%     caxis([-2 2])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
    ...
    uppara_cold_data_interp = uppara_cold_data_interp-uparacold0;
    upperp_cold_data_interp = upperp_cold_data_interp-uperpcold0;
...
%     [upx_cold_data_interp] = n_cold_data_interp.*interpolate_Data(up_cold_data,1,i,nx,nz,X,Z,X2,Z2)./va;
    %         upx_mixed_data_interp=upx_mixed_data_interp.*((n_mixed_data_interp > nFS_threshold))./va;
    %clear up_mixed_data_interp
    pcolor(uppara_cold_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upx_cold_data_interp
    colorbar; title('SW \Deltanvpara','FontSize', 14)
    %shading interp
%     caxis([8 16])
caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
...
%     [upy_cold_data_interp] = n_cold_data_interp.*interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2)./va;
    %         upy_cold_data_interp=upy_cold_data_interp.*((n_mixed_data_interp > nFS_threshold))./va;
    %clear up_mixed_data_interp
    pcolor(upperp_cold_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upy_cold_data_interp
    colorbar; title('SW \Deltanvperp','FontSize', 14)
    %shading interp
%     caxis([-2 2])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
...
%     plot10=subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    axes(ha(6));
    [upx_mixed_data_interp] = n_mixed_data_interp.*interpolate_Data(up_mixed_data,1,i,nx,nz,X,Z,X2,Z2);
    upx_mixed_data_interp=n_mixed_data_interp.*upx_mixed_data_interp.*((n_mixed_data_interp > nFS_threshold))./va;
    %clear up_mixed_data_interp
    pcolor(upx_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upx_mixed_data_interp
    colorbar; title('FS nvx','FontSize', 14)
    %shading interp
%      caxis([-30 30])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XTickLabel',[]); 
...
%     plot11=subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    ...
    %clear up_mixed_data_interp
    pcolor(upy_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upy_mixed_data_interp
    colorbar; title('FS nvy','FontSize', 14)
    %shading interp
%     caxis([-15 15])
caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
...
%     plot10=subplot(plot_height,plot_width,plot_number); plot_number = plot_number+1;
    ...
    axes(ha(9));
    pcolor(uppara_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upx_mixed_data_interp
    colorbar; title('FS nvpara','FontSize', 14)
    %shading interp
%      caxis([-30 30])
caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
...
%     upy_mixed_data_interp=upy_mixed_data_interp.*((n_mixed_data_interp > nFS_threshold))./va;
%     %clear up_mixed_data_interp
    pcolor(upperp_mixed_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear upy_mixed_data_interp
    colorbar; title('FS nvperp','FontSize', 14)
    %shading interp
%     caxis([-15 15])
caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
...
%     set(gca,'YTickLabel',[]); 
    ...
    %Tracking
    axes((ha(11)))
    set(ha(11),'Position',[0.1000 0.1000 3*0.1520+2.5*0.01 0.2400])
    delete(ha(12)); delete(ha(13))
     [Bxdata_interp] = (interpolate_Data(Bdata,1,i,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9);
    [Bydata_interp] = (interpolate_Data(Bdata,2,i,nx,nz,X,Z,X2,Z2)).*moverq./(5e-9);
    coneAngle = atand(abs(Bydata_interp)./abs(Bxdata_interp));
    pcolor(coneAngle(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp  %contour(Bmagdata,'color','k')
...
%     clear ndata_interp
    ...
    colorbar;
    %shading interp
    title('Bx-By Cone Angle','FontSize', 14); xlabel({'X';'[\lambda_i]'},'FontSize',14); ylabel({'Z';'[\lambda_i]'},'FontSize',14)
%     caxis([-1 1])
     ylim(ylimits); xlim([2 nx-3]);
%     axis square
    ...
    Jpara_data_interp = Jpara_data_interp - Jpara0;
    Jperp_data_interp = Jperp_data_interp - Jperp0;
...
%     Jxdata_interp = imgaussfilt(Jxdata_interp,filterNumber);
    pcolor(Jpara_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     clear Jxdata_interp
    colorbar; title('\DeltaJpara','FontSize', 14);%ylim(ylimits)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %      shading interp
%     caxis([-2 2])
caxis([-max(abs(caxis)) max(abs(caxis))])
...
%     [Jydata_interp] = interpolate_Data(Jdata,2,i,nx,nz,X,Z,X2,Z2);%.*ndata_interp.*alpha;
%     Jydata_interp = imgaussfilt(Jydata_interp,filterNumber);
    pcolor(Jperp_data_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp%contour(pmagdata,'color','k')
%     clear Jydata_interp
    colorbar; title('\DeltaJperp','FontSize', 14);%ylim(ylimits)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    %      shading interp
%     caxis([-2 2]);
caxis([-max(abs(caxis)) max(abs(caxis))])
