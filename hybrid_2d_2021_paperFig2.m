...
fig = figure('Position',[ 1 1 250+5*250 3*250]);
outputFolder = '180';
...
for i=1:stepInterval:nt
   ...
   %% Up Cold Para/Perp
    ...
    [uppara_cold_data_interp,upperp_cold_data_interp] = calculate_components(Bxdata_interp,Bydata_interp,Bzdata_interp,upx_cold_data_interp,upy_cold_data_interp,upz_cold_data_interp);
    uppara_cold_data_interp = (uppara_cold_data_interp-uparacold0).*10^3 * 10^-9.*q./nv0;
    upperp_cold_data_interp = (upperp_cold_data_interp-uperpcold0).*10^3 * 10^-9.*q./nv0;
...
%     set(gca,'XTickLabel',[]);
    ...
    axes(ha(6));
    [upx_mixed_data_interp] = abs(interpolate_Data(up_mixed_data,1,i,nx,nz,X,Z,X2,Z2));
    upx_mixed_data_interp=upx_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold))./va-2*FS_Ma.*((n_mixed_data_interp./n0 > nFS_threshold));
    ...
    colorbar; title('FS \Delta|vx|/va','FontSize', 14)
    %shading interp
    %      caxis([-30 30])
%     caxis([-max(abs(caxis-2*FS_Ma))-2*FS_Ma max(abs(caxis-2*FS_Ma))-2*FS_Ma])
    ...
    upy_mixed_data_interp=upy_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold))./va;
    ...
    colorbar; title('FS vy/va','FontSize', 14)
    %shading interp
    %     caxis([-15 15])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
    ...
    upz_mixed_data_interp=upz_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold))./va;
    ...
    uppara_mixed_data_interp = uppara_mixed_data_interp.*10^3 * 10^-9.*q./nv0;
    upperp_mixed_data_interp = upperp_mixed_data_interp.*10^3 * 10^-9.*q./nv0;
    axes(ha(9));
    pcolor(uppara_mixed_data_interp(nz_min:nz_max,nx_min:nx_max));  shading interp
    %     clear upx_mixed_data_interp
    colorbar; title('FS nqvpara/J0','FontSize', 14)
    %shading interp
    %      caxis([-30 30])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
    ...
    %     clear upy_mixed_data_interp
    colorbar; title('FS nqvperp/J0','FontSize', 14)
    %shading interp
    %     caxis([-15 15])
    caxis([-max(abs(caxis)) max(abs(caxis))])
    set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2); ylim(ylimits);xlim(xlimits)
...
%     [Jpara_data_interp,Jperp_data_interp] = calculate_components(Bxdata_interp,Bydata_interp,Bzdata_interp,Jxdata_interp,Jydata_interp,Jzdata_interp);
    Jpara_data_interp = (Jpara_data_interp - Jpara0).*moverq./J0;
    Jperp_data_interp = (Jperp_data_interp - Jperp0).*moverq./J0;
