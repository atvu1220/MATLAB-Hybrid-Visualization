...
zCellRange(i,:) = ZZ(zSC(i)-neighboring:zSC(i)+neighboring,1).*oneCelllength;
end
% ParticleInCellCount = zeros(nt,1);
% ParticleInCellPos=zeros(nt,procnum*nmax,3);
% % % % % % % % % % ParticleInCellVel=zeros(nt,procnum*nmax,2);
% % % % % % % % % % ParticleInCellMixed=zeros(nt,procnum*nmax,1);
% % % % % % % % % % 
% % % % % % % % % % 
% % % % % % % % % % ParticleInCellVel = allv(allx(1,:,1) > xCellRange(1) & allx(1,:,1) < xCellRange(end)) ;
% % % % % % % % % % ParticleInCellMixed = 
% % % % % % % % % % for t=1:nt %Per Each Time
% % % % % % % % % %     for i=1:proc*nmax% Per each Particle
% % % % % % % % % %         if ((allx(t,i,1) > xCellRange(1) && allx(t,i,1) < xCellRange(end)) &&...
% % % % % % % % % %                 (allx(t,i,2) > zCellRange(1) && allx(t,i,2) < zCellRange(end)))
...
% % % % % % % % % % % [ndata] = read_Plasma('n',nt_n,nx,ny,nz,outputDirectory);


plot_width= 1;
plot_height= length(xSC);
plot_number = 1;
...
if length(xSC) > 1
    fig = figure('Position',[ 1 1 400 400*length(xSC)]);
else
    fig = figure('Position',[ 1 1 600 600*length(xSC)]);
end
colormap(A)
set(gcf,'color','w');

fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_vDist_',xDir,yDir,'_Run',RunNumber);%,'_x(',string(xSC),')_z(',string(zSC),')');
v = VideoWriter(fileName);v.FrameRate= 1;
open(v);
%Plotting distribution
for t=1:nt
    if length(xSC) > 1
        [ha,~] = tight_subplot(plot_height,plot_width,[0.00 0.01],0.15,0.0);
    else
        [ha,~] = tight_subplot(plot_height,plot_width,[0.00 0.00],0.15,0.025);
    end
% % % % % % % % % % % % % %       % %5Get Cell
...
% % % % % % % % % % % % % %     axes(ha(1));
% % % % % % % % % % % % % %     [ndata_interp] = interpolate_Data(ndata,1,t*20,nx,nz,X,Z,X2,Z2);
% % % % % % % % % % % % % %     imagesc(((ndata_interp(2:nz-1,2:nx-1)))/(1e15));  hold on;
...
% % % % % % % % % % % % % %     colorbar;
% % % % % % % % % % % % % %     %shading interp
% % % % % % % % % % % % % %     title('Solar Wind Density','FontSize', 14)
% % % % % % % % % % % % % %     caxis([0 10])
% % % % % % % % % % % % % %     
% % % % % % % % % % % % % %     
% % % % % % % % % % % % % %     %
...
%     h = pcolor(X,Y,Cphi) ;
    ...
    c = colorbar;%('location', 'north');%colorbar;
    c.Limits = ([-8 7]);
    cbarTicks = c.Ticks;
...
%     title(strcat('Ion Velocity Distribution at t=',string(dt*t*outputSteps),' gyroperiods'),'FontSize',14);
    ...
    xlabel({xDir;'[M_A]'},'FontSize',14);
    ylabel({yDir;'[M_A]'},'FontSize',14)
    box on
%     
    ylim([-50 50])
    xlim([-50 50])
...
%     set(gcf,'color','w','Position',[100 100 800 800]);
    grid on
    set(ha(SC),'XMinorTick','off','TickDir','in','YMinorTick','off','linewidth',2)
%     xtickangle(45)
...
%     textboxPos = ha(SC).Position+ [0.3 0.3 0 0 ];
    ...
    yboundary = get(ha(SC),'ylim');
    xboundary = get(ha(SC),'xlim');
...
close(v)
fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/','v_dist_Run',RunNumber);
