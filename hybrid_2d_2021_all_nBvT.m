...
% ylimits = [ 2 nz-1]
% xlimits = [ 2 nx-3]
ylimits = [151 249];%[nz/2 - 49 nz/2 + 49]
xlimits = [101 199];%[(nx-2)/2-49 (nx-2)/2+49]
xlimits = [81 159];
% xlimits = [600+1 1000-1 ] ;
% ylimits = [100+1 800-1 ]  ;

moverq=1.0378e-8;
if pngSlice == 0
    fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/2d_Hybrid_Foreshock_all_Run_nBvT',RunNumber);
...
ndata_interp0 = imgaussfilt(ndata_interp0,filterNumber);
n0 = mean(ndata_interp0(floor(2):nz-1,2:end-1),'all');
n0 = imgaussfilt(n0,1);
...
Bxdata_interp0 = imgaussfilt(interpolate_Data(Bdata,1,1,nx,nz,X,Z,X2,Z2),filterNumber);
Bydata_interp0 = imgaussfilt(interpolate_Data(Bdata,2,1,nx,nz,X,Z,X2,Z2),filterNumber);
Bzdata_interp0 = imgaussfilt(interpolate_Data(Bdata,3,1,nx,nz,X,Z,X2,Z2),filterNumber);
Bmagdata0 = (Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2).^(1/2);
B0 = mean(Bmagdata0(floor(2):nz-1,2:end-1),'all');

[pxdata_interp0] = imgaussfilt(interpolate_Data(pdata,1,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
[pydata_interp0] = imgaussfilt(interpolate_Data(pdata,2,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
[pzdata_interp0] = imgaussfilt(interpolate_Data(pdata,3,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;

[Exdata_interp0] = imgaussfilt(interpolate_Data(Edata,1,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
[Eydata_interp0] = imgaussfilt(interpolate_Data(Edata,2,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
[Ezdata_interp0] = imgaussfilt(interpolate_Data(Edata,3,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;

[Jxdata_interp0] = imgaussfilt(interpolate_Data(Jdata,1,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
[Jydata_interp0] = imgaussfilt(interpolate_Data(Jdata,2,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
[Jzdata_interp0] = imgaussfilt(interpolate_Data(Jdata,3,1,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
...
for i=startTime:stepInterval:nt
    i
    [ha,~] = tight_subplot(plot_height,plot_width,[0.00 0.00],0.1,0.1);
    ...
    %% n 
    axes(ha(hacount)); hacount = hacount +1;
    [ndata_interp] = imgaussfilt(interpolate_Data(ndata,1,i,nx,nz,X,Z,X2,Z2))-0*ndata_interp0;
    pcolor(((ndata_interp(nz_min:nz_max,nx_min:nx_max)))/(n0));  hold on; shading interp
...
%     set(gca,'YTickLabel',[]);
    ...
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos,'Color','w');
    ylim(ylimits),xlim(xlimits);
%     title('\Deltan/n_0','FontSize', 14)
    caxis([0 4])
    ...
    pcolor(nFSdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on; shading interp
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos,'Color','w');
%         t = title('n_{fs}/n_0','FontSize', 14);   
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis([0 0.24])
%     if max(nFSdata_interp,[],'all') > 0.4
%         caxis([0. 0.3])
%     end
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    set(gca,'YTickLabel',[]);
%     xlabel('n_{fs}/n0','FontSize', 14)
...
%     yticks([ 150 200 250 ])
    ...
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos,'Color','w');
%         title('T/T_0','FontSize', 14);
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 100)
%         caxis([0 max(abs(cbarLim))])
%     else
        caxis([0 150]);
%     end
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
...
%     xlabel('T/T_0','FontSize', 14)
    ...
    deltaB = (Bmagdata - Bmagdata0)./(5e-9./moverq);
    
    axes(ha(hacount)); hacount = hacount +1;
    pcolor((Bxdata_interp(nz_min:nz_max,nx_min:nx_max)-Bxdata_interp0(nz_min:nz_max,nx_min:nx_max))./B0); hold on; shading interp  %contour(Bmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);
%     title('\DeltaBx/B_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 0.5)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
%     else
        caxis([-1.05 1.05]);
%     end
    %ylim(ylimits)
...
%     set(gca,'XTickLabel',[]); set(gca,'YTickLabel',[]);
    ...
    axes(ha(hacount)); hacount = hacount +1;
    pcolor((Bydata_interp(nz_min:nz_max,nx_min:nx_max)-Bydata_interp0(nz_min:nz_max,nx_min:nx_max))./B0); hold on; shading interp  %contour(Bmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);
%     title('\DeltaBy/B_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 0.5)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
...
%     end
    ...
    axes(ha(hacount)); hacount = hacount +1;
    pcolor((Bzdata_interp(nz_min:nz_max,nx_min:nx_max)-Bzdata_interp0(nz_min:nz_max,nx_min:nx_max))./B0); hold on; shading interp  %contour(Bmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);
%     title('\DeltaBz/B_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) >0.25)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
%     else
        caxis([-0.25 0.25]);
%     end
    ...
    %% P
    [pxdata_interp] = imgaussfilt(interpolate_Data(pdata,1,i,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
    [pydata_interp] = imgaussfilt(interpolate_Data(pdata,2,i,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
    [pzdata_interp] = imgaussfilt(interpolate_Data(pdata,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    Pmagdata = (pxdata_interp.^2 + pydata_interp.^2 + pzdata_interp.^2).^(1/2);
    Pdefldata=acosd(pxdata_interp./Pmagdata);
    deltaP = (Pmagdata./va - MA)./MA;
    
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((pxdata_interp(nz_min:nz_max,nx_min:nx_max)-pxdata_interp0(nz_min:nz_max,nx_min:nx_max))./va); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaUx/v_A','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 8)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
%     else
        caxis([-12 12]);
%     end
    ...
    % set(gca,'YTickLabel',[]);
%     xlabel('\DeltaUx','FontSize', 14)
    
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((pydata_interp(nz_min:nz_max,nx_min:nx_max)-pydata_interp0(nz_min:nz_max,nx_min:nx_max))./va); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaUy/v_A','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 4)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
%     else
        caxis([-2 2]);
%     end
    ...
    %     ylabel('Z','FontSize',14)
    set(gca,'YTickLabel',[]);
%     xlabel('\DeltaUy','FontSize', 14)
    
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((pzdata_interp(nz_min:nz_max,nx_min:nx_max)-pzdata_interp0(nz_min:nz_max,nx_min:nx_max))./va); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaUz/v_A','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 4)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
...
%     end
    ...
    %     ylabel('Z','FontSize',14)
    set(gca,'YTickLabel',[]);
%     xlabel('\DeltaUz','FontSize', 14)    
%% E
    [Exdata_interp] = imgaussfilt(interpolate_Data(Edata,1,i,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
    [Eydata_interp] = imgaussfilt(interpolate_Data(Edata,2,i,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
    [Ezdata_interp] = imgaussfilt(interpolate_Data(Edata,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((Exdata_interp(nz_min:nz_max,nx_min:nx_max)-Exdata_interp0(nz_min:nz_max,nx_min:nx_max))./E0); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaEx/E_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > .2)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
%     else
        caxis([-5.5 5.5]);
%     end
...
%     xlabel('\DeltaUx','FontSize', 14)
    ...
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((Eydata_interp(nz_min:nz_max,nx_min:nx_max)-Eydata_interp0(nz_min:nz_max,nx_min:nx_max))./E0); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaEy/E_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > .2)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
...
%     xlabel('\DeltaUy','FontSize', 14)
    ...
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((Ezdata_interp(nz_min:nz_max,nx_min:nx_max)-Ezdata_interp0(nz_min:nz_max,nx_min:nx_max))./E0); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaEz/E_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > .2)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
...
%% J
    [Jxdata_interp] = imgaussfilt(interpolate_Data(Jdata,1,i,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
    [Jydata_interp] = imgaussfilt(interpolate_Data(Jdata,2,i,nx,nz,X,Z,X2,Z2),filterNumber);%./va-MA;
    [Jzdata_interp] = imgaussfilt(interpolate_Data(Jdata,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((Jxdata_interp(nz_min:nz_max,nx_min:nx_max)-Jxdata_interp0(nz_min:nz_max,nx_min:nx_max))./J0); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%     title('\DeltaJx/J_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    caxis(40*[-0.0099 0.0099])  
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > .5)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
%     else
        caxis([-0.41 0.41]);
%     end
    ...
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2)
    %     ylabel('Z','FontSize',14)
    % set(gca,'YTickLabel',[])00
%     xlabel('\DeltaUx','FontSize', 14)
    
    axes(ha(hacount)); hacount = hacount+1;
    pcolor((Jydata_interp(nz_min:nz_max,nx_min:nx_max)-Jydata_interp0(nz_min:nz_max,nx_min:nx_max))./J0); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaJy/J_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 0.25)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
...
%     xlabel('\DeltaUy','FontSize', 14)
    ...
    axes(ha(hacount));% hacount = hacount+1;
    pcolor((Jzdata_interp(nz_min:nz_max,nx_min:nx_max)-Jzdata_interp0(nz_min:nz_max,nx_min:nx_max))./J0); hold on; shading interp %contour(pmagdata,'color','k')
%     yticks([ 150 200 250 ])
    c = colorbar('location', 'north');
    cbarPos = get(c,'Position');
    cbarPos = cbarPos(3).*[0.1,0,0,0] + cbarPos(4).*[0,0.75,0,0] + cbarPos.*[1,1,0.8,0.25];
    set(c,'Position',cbarPos);%,'color','w');
%         title('\DeltaJz/J_0','FontSize', 14)
    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    cbarLim = get(c,'Limits');
%     if max(abs(cbarLim) > 0.125)
%         caxis([-max(abs(cbarLim)) max(abs(cbarLim))])
%     else
        caxis([-.15 .15]);
%     end
...
%     xlabel('\DeltaUz','FontSize', 14)  
    ...
    if i==pngSlice
        fileNamePNG = strcat('/import/c1/DAYSIDE/atvu/Runs/Paper3/','2D_Run',RunNumber,'_t',string(pngSlice));
