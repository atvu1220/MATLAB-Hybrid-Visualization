...
real:: vth2, vx, vy, vz, va, va_x, Temp, Tempcalc, pl_beta(nx,ny,nz), fitdist,randpop, EvBx, EvBy,EvBz,theta2,phi2, vxring, vyring, vzring
integer:: l,m,i,j,k
  ...
  !      vp(l,3) = FSThermalRatio*vz                       + ( up(i,j,k,3)*( b1(i,j,k,2)*b1(i,j,k,2) + b1(i,j,k,1)*b1(i,j,k,1) ) ) / ( ( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) )
     ...
     !   vp(l,3) = 												    + FSThermalRatio*vz + ExB(i,j,k,3)                 
  	!write(*,*)
        ...
        !vp(l,1) = -FSDriftSpeed*va_f*va*( b1(i,j,k,1) / (  sqrt( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) ) ) + FSThermalRatio*vx + ( EvBy*b1(i,j,k,3) - EvBz*b1(i,j,k,2) ) / ( ( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) )
        !vp(l,2) = -FSDriftSpeed*va_f*va*( b1(i,j,k,2) / (  sqrt( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) ) ) + FSThermalRatio*vy - sqrt(2.0)/2.0*FSThermalRatio*vth2 - ( EvBx*b1(i,j,k,3) - EvBz*b1(i,j,k,1) ) / ( ( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) )
        !vp(l,3) = -FSDriftSpeed*va_f*va*( b1(i,j,k,3) / (  sqrt( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) ) ) + FSThermalRatio*vz + sqrt(2.0)/2.0*FSThermalRatio*vth2 + ( EvBx*b1(i,j,k,2) - EvBy*b1(i,j,k,1) ) / ( ( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) )
...
!        vp(l,3) = -FSDriftSpeed*va_f*va*( b0(i,j,k,3) / (  sqrt( b0(i,j,k,1)**2+b0(i,j,k,2)**2+b0(i,j,k,3)**2 ) ) ) + FSThermalRatio*vz + sqrt(2.0)/2.0*FSThermalRatio*vth2 + ( EvBx*b0(i,j,k,2) - EvBy*b0(i,j,k,1) ) / ( ( b0(i,j,k,1)**2+b0(i,j,k,2)**2+b0(i,j,k,3)**2 ) )
	...
	!Ring beam +exB
        ...
        vyring = 1.5*1.0*FSThermalRatio*vth2
        vzring = 1.5*1.0*FSThermalRatio*vth2	
       ! montecarlo = 0
