...
moverq=1.0378e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_Jperp_noewithdeltau',RunNumber,'_',string(xcut));
...
%ExB Speed
%Calculate E=-vxB;
vExB_x = Bydata_interp0.^2                  ./ (Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2) .* MA;
vExB_y = -Bxdata_interp0.*Bydata_interp0    ./ (Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2) .* MA;
for k=nz/2+1:nz
...
upx_mixed_bulk = upx_mixed_bulk + (1-ele)*vExB_x;
upy_mixed_bulk = upy_mixed_bulk + (1-ele)*vExB_y;
...
ue0 = (Exdata_interp0.^2 + Eydata_interp0.^2 + Ezdata_interp0.^2 ).^(1/2) ./ (Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2).^(1/2);
[uex0,uey0,uez0] = calculate_2Dcross(Exdata_interp0,Eydata_interp0,Ezdata_interp0,Bxdata_interp0,Bydata_interp0,Bzdata_interp0);
% EBmag = (Exdata_interp0.^2 + Eydata_interp0.^2 + Ezdata_interp0.^2)./(1/2).*(Bxdata_interp0.^2 + Bydata_interp0.^2 + Bzdata_interp0.^2).^(1/2);
ue0mag = (uex0.^2 + uey0.^2 + uez0.^2).^(1/2);
uex0 = uex0./ue0mag;
uey0 = uey0./ue0mag;
uez0 = uez0./ue0mag;
...
ppx_electron0 = (n_cold_data_interp0).*ue0.*uex0;
ppy_electron0 = (n_cold_data_interp0).*ue0.*uey0;
ppz_electron0 = (n_cold_data_interp0).*ue0.*uez0;
...
% uex_cold0 = upx_cold_data_interp0 - curlBx_interp0*moverq./ (1000*4*pi*10^-7 .* q .* n_cold_data_interp0*10^-9)./1000; %in km/s
% uey_cold0 = upy_cold_data_interp0 - curlBy_interp0*moverq./ (1000*4*pi*10^-7 .* q .* n_cold_data_interp0*10^-9)./1000;
% uez_cold0 = upz_cold_data_interp0 - curlBz_interp0*moverq./ (1000*4*pi*10^-7 .* q .* n_cold_data_interp0*10^-9)./1000;

% uex_mixed0 = upx_mixed_bulk.*va - curlBx_interp0*moverq./ (1000*4*pi*10^-7 .* q .* (0.12.*n0)*10^-9)./1000; %in km/s
% uey_mixed0 = upy_mixed_bulk.*va - curlBy_interp0*moverq./ (1000*4*pi*10^-7 .* q .* (0.12.*n0)*10^-9)./1000; %in km/s
% uez_mixed0 = upz_mixed_bulk.*va - curlBz_interp0*moverq./ (1000*4*pi*10^-7 .* q .* (0.12.*n0)*10^-9)./1000; %in km/s

[ppx_cold_data_interp0] = 0.996.*n_cold_data_interp0.*(upx_cold_data_interp0);%-ele*uex_cold0);
[ppy_cold_data_interp0] = 0.996.*n_cold_data_interp0.*(upy_cold_data_interp0);%-ele*uey_cold0);
[ppz_cold_data_interp0] = 0.996.*n_cold_data_interp0.*(upz_cold_data_interp0);%-ele*uez_cold0);
...
% [Jpara0,Jperp0] = calculate_components(Bxdata_interp0(nz_min:nz_max,nx_min-1:nx_max-1),Bydata_interp0(nz_min:nz_max,nx_min-1:nx_max-1),Bzdata_interp0(nz_min:nz_max,nx_min-1:nx_max-1),Jxdata_interp0,Jydata_interp0,Jzdata_interp0);
...
for i=1:stepInterval:nt
    ...
    Bxdata_interp_y1 = imgaussfilt(Bxdata_interp_y1,filterNumber);
    ...
    Bydata_interp_y1 = imgaussfilt(Bydata_interp_y1,filterNumber);
    ...
    Bzdata_interp_y1 = imgaussfilt(Bzdata_interp_y1,filterNumber);
    ...
    ue = (Exdata_interp.^2 + Eydata_interp.^2 + Ezdata_interp.^2 ).^(1/2) ./ (Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2).^(1/2);
    [uex,uey,uez] = calculate_2Dcross(Exdata_interp,Eydata_interp,Ezdata_interp,Bxdata_interp,Bydata_interp,Bzdata_interp);
%     EBmag = (Exdata_interp.^2 + Eydata_interp.^2 + Ezdata_interp.^2)./(1/2).*(Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2).^(1/2);
    uemag = (uex.^2 + uey.^2 + uez.^2).^(1/2);
    uex = uex./uemag;
    uey = uey./uemag;
    uez = uez./uemag;
    ...
    ppx_electron = (n_cold_data_interp+n_mixed_data_interp).*ue.*uex;
    ppy_electron = (n_cold_data_interp+n_mixed_data_interp).*ue.*uey;
    ppz_electron = (n_cold_data_interp+n_mixed_data_interp).*ue.*uez;
    

    
    [upparax_electron,upparay_electron,upparaz_electron,upperpx_electron,upperpy_electron,upperpz_electron] = calculate_componentsVector(Bxdata_interp,Bydata_interp,Bzdata_interp,ppx_electron,ppy_electron,ppz_electron);
    [upparax_electron0,upparay_electron0,upparaz_electron0,upperpx_electron0,upperpy_electron0,upperpz_electron0] = calculate_componentsVector(Bxdata_interp,Bydata_interp,Bzdata_interp,ppx_electron0,ppy_electron0,ppz_electron0);
    
    
    delta_upparax_electron = (upparax_electron-upparax_electron0)* 10^-9.*q./nv0;
    delta_upparay_electron = (upparay_electron-upparay_electron0)* 10^-9.*q./nv0;
    delta_upparaz_electron = (upparaz_electron-upparaz_electron0)* 10^-9.*q./nv0;
    delta_upperpx_electron = (upperpx_electron-upperpx_electron0)* 10^-9.*q./nv0;
    delta_upperpy_electron = (upperpy_electron-upperpy_electron0)* 10^-9.*q./nv0;
    delta_upperpz_electron = (upperpz_electron-upperpz_electron0)* 10^-9.*q./nv0;
...
%     
    ...
    delta_upparax_cold = upparax_cold-upparax_cold0;
    delta_upparay_cold = upparay_cold-upparay_cold0;
    delta_upparaz_cold = upparaz_cold-upparaz_cold0;
    delta_upperpx_cold = upperpx_cold-upperpx_cold0;
    delta_upperpy_cold = upperpy_cold-upperpy_cold0;
    delta_upperpz_cold = upperpz_cold-upperpz_cold0;
    
    %     uex_cold = upx_cold_data_interp - curlBx_interp*moverq./ (1000*4*pi*10^-7 .* q .* n_cold_data_interp*10^-9)./1000; %in km/s
    %     uey_cold = upy_cold_data_interp - curlBy_interp*moverq./ (1000*4*pi*10^-7 .* q .* n_cold_data_interp*10^-9)./1000;
    %     uez_cold = upz_cold_data_interp - curlBz_interp*moverq./ (1000*4*pi*10^-7 .* q .* n_cold_data_interp*10^-9)./1000;
    
    [ppx_cold_data_interp] = n_cold_data_interp.*(upx_cold_data_interp);%-ele*uex_cold);
    [ppy_cold_data_interp] = n_cold_data_interp.*(upy_cold_data_interp);%-ele*uey_cold);
    [ppz_cold_data_interp] = n_cold_data_interp.*(upz_cold_data_interp);%-ele*uez_cold);
    ...
    upy_cold_data_interp = imgaussfilt(interpolate_Data(up_cold_data,2,i,nx,nz,X,Z,X2,Z2),filterNumber);
    upz_cold_data_interp = imgaussfilt(interpolate_Data(up_cold_data,3,i,nx,nz,X,Z,X2,Z2),filterNumber);
    Ex = Bydata_interp.*upz_cold_data_interp - Bzdata_interp.*upy_cold_data_interp;
    Ey = Bzdata_interp.*upx_cold_data_interp - Bxdata_interp.*upz_cold_data_interp;
    Ez = Bxdata_interp.*upy_cold_data_interp - Bydata_interp.*upx_cold_data_interp;
    ...
    %nsw*q(vperpsw) + nfs*qvperpfs - ntot*qve (total E)
    
    
    upx_mixed_data_interp = imgaussfilt(interpolate_Data(up_mixed_data,1,i,nx,nz,X,Z,X2,Z2),filterNumber);%-(1-ele)*vExBx;
    upy_mixed_data_interp = imgaussfilt(interpolate_Data(up_mixed_data,2,i,nx,nz,X,Z,X2,Z2),filterNumber);%-(1-ele)*vExBy;
    upz_mixed_data_interp = imgaussfilt(interpolate_Data(up_mixed_data,3,i,nx,nz,X,Z,X2,Z2),filterNumber);%-(1-ele)*vExBz;
    ...
    delta_upparax_mixed = upparax_mixed - upparax_mixed0;
    delta_upparay_mixed = upparay_mixed - upparay_mixed0;
    delta_upparaz_mixed = upparaz_mixed - upparaz_mixed0;
    delta_upperpx_mixed = upperpx_mixed - upperpx_mixed0;
    delta_upperpy_mixed = upperpy_mixed - upperpy_mixed0;
    delta_upperpz_mixed = upperpz_mixed - upperpz_mixed0;
...
%     uex_mixed = upx_mixed_data_interp - curlBx_interp*moverq./ (1000*4*pi*10^-7 .* q .* (n_mixed_data_interp)*10^-9)./1000; %in km/s
%     uey_mixed = upy_mixed_data_interp - curlBy_interp*moverq./ (1000*4*pi*10^-7 .* q .* (n_mixed_data_interp)*10^-9)./1000; %in km/s
%     uez_mixed = upz_mixed_data_interp - curlBz_interp*moverq./ (1000*4*pi*10^-7 .* q .* (n_mixed_data_interp)*10^-9)./1000; %in km/s
     
    [ppx_mixed_data_interp] = n_mixed_data_interp.*(upx_mixed_data_interp);%-ele*uex_mixed);
    ppx_mixed_data_interp= ppx_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold));
    [ppy_mixed_data_interp] = n_mixed_data_interp.*(upy_mixed_data_interp);%-ele*uey_mixed);
    ppy_mixed_data_interp= ppy_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold));
    [ppz_mixed_data_interp] = n_mixed_data_interp.*(upz_mixed_data_interp);%-ele*uez_mixed);
    ppz_mixed_data_interp= ppz_mixed_data_interp.*((n_mixed_data_interp./n0 > nFS_threshold));
    
    ppx_mixed_data_interp0 = 0.12*n0.*(upx_mixed_bulk.*va-0*ele*uex_mixed0).*((n_mixed_data_interp./n0 > nFS_threshold));
    ppy_mixed_data_interp0 = 0.12*n0.*(upy_mixed_bulk.*va-0*ele*uey_mixed0).*((n_mixed_data_interp./n0 > nFS_threshold));
    ppz_mixed_data_interp0 = 0.12*n0.*(upz_mixed_bulk.*va-0*ele*uez_mixed0).*((n_mixed_data_interp./n0 > nFS_threshold));
...
%     [Jxdata_interp,Jydata_interp,Jzdata_interp] = calculate_curlB(Bxdata_interp,Bxdata_interp_y1,Bydata_interp,Bydata_interp_y1,Bzdata_interp,Bzdata_interp_y1,lambda_i,nx_min,nx_max,nz_min,nz_max);
...
%     [Jparax,Jparay,Jparaz,Jperpx,Jperpy,Jperpz] = calculate_componentsVector(Bxdata_interp(nz_min:nz_max,nx_min-1:nx_max-1),Bydata_interp(nz_min:nz_max,nx_min-1:nx_max-1),Bzdata_interp(nz_min:nz_max,nx_min-1:nx_max-1),Jxdata_interp,Jydata_interp,Jzdata_interp);
%     [Jparax0,Jparay0,Jparaz0,Jperpx0,Jperpy0,Jperpz0] = calculate_componentsVector(Bxdata_interp(nz_min:nz_max,nx_min-1:nx_max-1),Bydata_interp(nz_min:nz_max,nx_min-1:nx_max-1),Bzdata_interp(nz_min:nz_max,nx_min-1:nx_max-1),Jxdata_interp0,Jydata_interp0,Jzdata_interp0);
...
[Jparax0,Jparay0,Jparaz0,Jperpx0,Jperpy0,Jperpz0] = calculate_componentsVector(Bxdata_interp,Bydata_interp,Bzdata_interp,Jxdata_interp0,Jydata_interp0,Jzdata_interp0);
    ...
    %Totals
    Jperpx_tot = delta_cold_perpx(2:end-1,2:end-1) + 1*jDx_cold + delta_mixed_perpx(2:end-1,2:end-1) + 1*jDx_mixed;
    Jperpy_tot = delta_cold_perpy(2:end-1,2:end-1) + 1*jDy_cold + delta_mixed_perpy(2:end-1,2:end-1) + 1*jDy_mixed;
    
    %% Plotting
%     axes(ha(axes_count));axes_count=axes_count+1;
%     Bmagdata = (Bxdata_interp.^2 + Bydata_interp.^2 + Bzdata_interp.^2).^(1/2);
%     hold off;
%     plot(smooth(Bxdata_interp(:,xcut).*moverq./(5e-9)),'b','linewidth',1)
%     hold on
%     plot(smooth(Bydata_interp(:,xcut).*moverq./(5e-9)),'g','linewidth',1)
%     plot(smooth(Bzdata_interp(:,xcut).*moverq./(5e-9)),'r','linewidth',1)
%     plot(Bmagdata(:,xcut).*moverq./(5e-9),'--k')
%     hold off
%     ylabel({'B/B0'},'FontSize',14)
%     xlim(zrange)
%     if max(abs(ylim)) > 1.25
%         ylim([-0.30 max(abs(ylim))])
%     else
%         ylim([-0.30 1.25])
%     end
%     legend('Bx','By','Bz','Btotal','Location','southeast','Orientation','horizontal'); legend('boxoff')
%     set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    ...
    plot(smooth(jDx_mixed(:,xcut)),'linewidth',1)
    hold off
    ylabel({'Jperpx/J0'},'FontSize',14)
    xlim(zrange)
    Alegend = legend('\Deltanqv_{sw}','jD_{sw}','\Deltanqv_{fs}','jD_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
%     Alegend = legend('\Deltanqv_{sw}','\Deltanqv_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    set(gca,'xminortick','on','yminortick','on','linewidth',1.5,'XTickLabel',[])
    
    axes(ha(axes_count));axes_count=axes_count+1;
    yyaxis left
    plot(smooth(Jperpx_tot(:,xcut)),'b','linewidth',1)
    ...
    Clegend = legend('Jperptot','\DeltaJperp','Location','northwest','Orientation','horizontal'); legend('boxoff')
    ...
    plot(smooth(jDy_mixed(:,xcut)),'linewidth',1)
    hold off
    ylabel({'Jperpy/J0'},'FontSize',14)
    xlim(zrange)
    Blegend = legend('\Deltanqv_{sw}','jD_{sw}','\Deltanqv_{fs}','jD_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
%     Blegend = legend('\Deltanqv_{sw}','\Deltanqv_{fs}','Location','northwest','Orientation','horizontal'); legend('boxoff')
    ...
    yyaxis left
    plot(smooth(Jperpy_tot(:,xcut)),'b','linewidth',1)
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    yyaxis right
    ...
    Dlegend = legend('Jperptot','\DeltaJperp','Location','northwest','Orientation','horizontal'); legend('boxoff')
