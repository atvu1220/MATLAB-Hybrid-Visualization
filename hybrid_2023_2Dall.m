...
if length(TD) == 2
    xlimits = [299 901];
    ylimits = [49 551];
    fileName = strcat('/home/andrew/Documents/output/2d_Hybrid_all_Run',RunNumber,'_bothTD2_t=',string(floor(startFrame/2)));

else
    xlimits = [826-302 826];
    ylimits = [TD-126 TD+126];
    if TD == 150
        fileName = strcat('/home/andrew/Documents/output/2d_Hybrid_all_Run',RunNumber,'_botTD2_t=',string(floor(startFrame/2)));

    elseif TD == 450
        fileName = strcat('/home/andrew/Documents/output/2d_Hybrid_all_Run',RunNumber,'_topTD2_t=',string(floor(startFrame/2)));
    end
end
...
Bx0 = interpolate_Data(Bdata,1,1,nx,nz,X,Z,X2,Z2);
By0 = interpolate_Data(Bdata,2,1,nx,nz,X,Z,X2,Z2);
...
for i=startFrame:stepInterval:stopFrame
    ...
    % Bz
    axes(ha(axes_count));axes_count=axes_count+1;
    [Bzdata_interp] = (interpolate_Data(Bdata,3,i,nx,nz,X,Z,X2,Z2)-0*interpolate_Data(Bdata,3,1,nx,nz,X,Z,X2,Z2)).*moverq./Btot0;
    ...
    % Ex
    axes(ha(axes_count));axes_count=axes_count+1;
    [Exdata_interp] = interpolate_Data(Edata,1,i,nx,nz,X,Z,X2,Z2).*moverq ./E0;
    Exdata_interp = imgaussfilt(Exdata_interp,filterNumber);
    imagesc(Exdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on;
    colorbar;
    ...
    % Ey
    axes(ha(axes_count));axes_count=axes_count+1;
    [Eydata_interp] = interpolate_Data(Edata,2,i,nx,nz,X,Z,X2,Z2).*moverq ./E0;
    Eydata_interp = imgaussfilt(Eydata_interp,filterNumber);
    imagesc(Eydata_interp(nz_min:nz_max,nx_min:nx_max)); hold on;
    colorbar;

    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2,'fontsize',18)
    set(gca,'YTickLabel',[],'fontsize',18); set(gca,'XTickLabel',[],'fontsize',18)
    title('E_y/E_0','FontSize', 28);
    caxis(0.5*[-1 1])
    ...
    % Ez
    axes(ha(axes_count));axes_count=axes_count+1;
    [Ezdata_interp] = (interpolate_Data(Edata,3,i,nx,nz,X,Z,X2,Z2)-1*Ez0).*moverq ./E0;
    Ezdata_interp = imgaussfilt(Ezdata_interp,filterNumber);
    imagesc(Ezdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on;
    colorbar;

    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2,'fontsize',18)
    set(gca,'YTickLabel',[],'fontsize',18); set(gca,'XTickLabel',[],'fontsize',18)
    title('\DeltaE_z/E_0','FontSize', 28);
    caxis(2*[-1 1])
    ...
    % Jx
    axes(ha(axes_count));axes_count=axes_count+1;
    [Jxdata_interp] = moverq.*interpolate_Data(Jdata,1,i,nx,nz,X,Z,X2,Z2)./J0;
    Jxdata_interp = imgaussfilt(Jxdata_interp,filterNumber);
    imagesc(Jxdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on;
    colorbar;

    set(gca,'YDir','normal');ylim(ylimits),xlim(xlimits);
    set(gca,'XMinorTick','on','TickDir','out','YMinorTick','on','linewidth',2,'fontsize',18)
    ylabel('Z','FontSize',18); xlabel('X','FontSize',18)
    title('J_x/J_0','FontSize', 28);
    caxis(0.05*[-1 1])
    ...
    % Jy
    axes(ha(axes_count));axes_count=axes_count+1;
    [Jydata_interp] = moverq.*interpolate_Data(Jdata,2,i,nx,nz,X,Z,X2,Z2)./J0;
    Jydata_interp = imgaussfilt(Jydata_interp,filterNumber);
    imagesc(Jydata_interp(nz_min:nz_max,nx_min:nx_max)); hold on;
    colorbar;
    ...
    % Jz
    axes(ha(axes_count));axes_count=axes_count+1;
    [Jzdata_interp] = moverq.*interpolate_Data(Jdata,3,i,nx,nz,X,Z,X2,Z2)./J0;
    Jzdata_interp = imgaussfilt(Jzdata_interp,filterNumber);
    imagesc(Jzdata_interp(nz_min:nz_max,nx_min:nx_max)); hold on;
    colorbar;
