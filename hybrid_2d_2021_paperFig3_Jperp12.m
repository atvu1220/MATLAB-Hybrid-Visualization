...
zrange = [nz/2 - 99, nz/2 + 99];
zrange = [2 nz-2];
% zrange = [100 500]
moverq=1.0378e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_Jperp12_',RunNumber,'_',string(xcut));
...
%     
    ...
    delta_Jparax = (Jperpx-del*Jparax0).*moverq./J0;
...
%     legend('\Deltau_{sw}','\Deltau_{e}','u_{fs}','Location','northeast','Orientation','horizontal'); legend('boxoff')
    ...
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    if del==1 
       ylabel({'\DeltaJ_{\perpy}/J0'},'FontSize',14) 
    else
    ylabel({'J\perpy/J0'},'FontSize',14)
    end
    xlim(zrange)
