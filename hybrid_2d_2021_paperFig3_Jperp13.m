...
zrange = [400 - 99, 400+99];

moverq=1.0378e-8;
fileName = strcat('/import/c1/DAYSIDE/atvu/Runs/2d_Hybrid_Foreshock_Jpara13_',RunNumber,'_',string(xcut));
...
%     
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    plot(smooth(Jparax_cold(:,xcut)),'b','linewidth',1)
    hold on
    plot(smooth((Jparax_mixed(:,xcut))),'r','linewidth',1)
    hold off
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    if del==1
...
%     legend('\Deltau_{sw}','\Deltau_{e}','u_{fs}','Location','northeast','Orientation','horizontal'); legend('boxoff')
    ...
    axes(ha(axes_count));axes_count=axes_count+1;
    plot(smooth(Jparay_cold(:,xcut)),'b','linewidth',1)
    hold on
    plot(smooth((Jparay_mixed(2:end-1,xcut))),'r','linewidth',1)
    hold off
    ylim([ -max(abs(ylim)) max(abs(ylim)) ])
    if del==1 
       ylabel({'\DeltaJ_{||y}/J0'},'FontSize',14) 
    else
    ylabel({'J||y/J0'},'FontSize',14)
    end
    xlim(zrange)
